...
- **`media` 생략(기본, 권장)**: 응답은 naver, google, meta, tiktok 순서로 **고정된 4개
  블록**이 빈 줄로 이어붙어 한 번에 온다 — `media_list`와 무관하게 항상 이 순서로 4개가
  온다. **1회 호출로 충분** — 매체마다 반복 호출하지 않는다.
  - 각 블록은 (a) `month:`/`as_of_date:`/`media:` 헤더 3줄 + 빈 줄 + 표, 또는 (b) 헤더
    없이 `"No {media} budget/target available for {month}."` 한 줄 중 하나다 — 표 없는
    블록은 `media:` 헤더가 없으므로 헤더 줄로 찾지 말고, **고정 순서대로 정확히 4개
    세그먼트로 나눠** 1번째=naver, 2번째=google, 3번째=meta, 4번째=tiktok로 배정한다.
  - 그 중 디스커버리로 받은 `media_list`에 있는 매체에 해당하는 세그먼트만 쓰고, 나머지
    (브랜드가 쓰지 않는 매체)는 버린다.
- **`media` 명시(선택)**: `"naver"|"google"|"meta"|"tiktok"` 중 하나를 넘기면 해당 매체
  블록 하나만 온다(단일 매체 동작은 이전과 동일, 변경 없음) — 표(행: cost/revenue/roas ×
  열: target|actual|progress_ratio) 또는 no-budget 한 줄 중 하나가 온다. 해당 매체 예산이
  전혀 없으면 리터럴 `"No {media} budget/target available for {month}."` 한 줄이 반환된다
  — **오류가 아니다**.
