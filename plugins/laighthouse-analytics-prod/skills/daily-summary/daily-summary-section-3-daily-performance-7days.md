...
**report_type:** `daily-summary` (항상 포함). 기준일 포함 최근 7일 일자별 광고 성과(광고비/
매출/ROAS 혼합 차트). 광고비·매출은 `metric_keys`의 cost·revenue 키 값이다.
...
- **이 응답은 section-4/5가 그대로 재사용한다** — section-4는 날짜별 revenue 키 합(전체 매출은
...
| 필드 | 값 |
|---|---|
| `ad_cost` | 날짜별: 매체 행(`media` non-null)의 cost 키 합 |
| `revenue` | 날짜별: 매체 행(`media` non-null)의 revenue 키 합 |
| `roas` | 날짜별: 매출 ÷ 광고비 × 100 (광고비 0인 날은 `null`) — 매체별 행을 합산했으므로 서버 비율 지표를 더하지 말고 원자 지표 합으로 계산한다 |
