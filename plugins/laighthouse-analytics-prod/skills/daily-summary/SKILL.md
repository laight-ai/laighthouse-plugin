...
> `daily-detailed`의 실제 프로덕션 실행에서 이 방식으로 섹션마다 파일을 만들다가 최종
> 저장/완료 메시지 전송 전에 툴호출 제한에 걸려 사용자에게 아무 결과도 보여주지 못한 사례가
> 있었다). 각 섹션의 HTML 문자열은 그 섹션을 만드는 같은 턴에서 바로 최종 문서 조합에 이어
> 쓴다 — 파일을 매개로 왕복하지 않는다. **최종 저장 시점에 딱 한 번**, 완성된 전체 HTML을 아래
> "실행 순서"에 정해진 저장 경로에 Write한다.
>
...
2. target/achievement 수치를 호출한다 — `mcp__laighthouse__get_target_progress_v2`를
   ...
   **google/meta/naver 세 번**(`media`만 바꿔) 호출한다
   ...
   4단계 호출의 airbridge 행에서 가져온다 — `get_target_progress_v2`의 `revenue` 행 `actual`은
...
3. ⏱ **필수 체크포인트 — 2단계 응답을 받은 즉시, 다음 단계로 넘어가기 전에 스켈레톤을 먼저
   ...
   끝까지 만들다가 저장 직전에 제한에 걸림)로 인한 실패가 있었다. 스켈레톤 게시는 선택 사항이
   아니라 이 순서상 반드시 거쳐야 하는 단계다.
...
7. `chart.umd.min.js`(약 208KB)를 Read해서 그 전체 내용을 `{CHART_JS_INLINE}` 자리에 그대로
   붙여넣어 `<script>...</script>` 태그 안에 인라인한다. 이 파일은 **항상** 인라인해서
   저장/표시할 HTML 안에 직접 포함시킨다 — 별도 자산 파일로 저장 폴더에 복사해두고 상대 경로
   `<script src="chart.umd.min.js">`로 참조하는 방식은 쓰지 않는다.
   ⚠️ **2026-08-09 `creative-detailed` 실사용 확인**: 한때 이 방식(자산 파일 1회 복사 +
   상대 경로 참조)을 시도했으나, 샌드박스 출력 디렉터리(`/mnt/user-data/outputs/` 등)에
   저장된 리포트를 미리보기하는 환경에서 HTML과 `chart.umd.min.js`가 서로 별개의 다운로드
   가능한 파일로 분리되어 표시되며 상대 경로 `<script src>`가 그 형제 `.js` 파일을 실제로
   로드하지 못해 **모든 차트가 깨져(빈 캔버스) 렌더링**되는 것을 실제 운영 환경에서 확인함.
   토큰 절약보다 차트가 실제로 렌더링되는 것이 우선이므로, 매 리포트마다 208KB를 통째로
   인라인하는 방식으로 되돌린다.
...
8. 아래 **보고서 골격**에 섹션들을 삽입해 렌더링한다. 완성된 HTML은 호스트가 지원하는 경로
   전부에 낸다 — 단, 위 7단계 규칙대로 채팅 내부 표시 자체가 지원되지 않는 호스트에서는 그
   경로를 만들지 않는다:
...
> 누적되어 보고서 생성이 느려진다. "2단계 먼저, 4단계 나중"이라는 실행 순서상의 서술은 데이터
> 의존성이 아니라 설명 편의상 나눈 순서일 뿐이다(3단계 스켈레톤 게시는 2단계 응답만으로
> 진행하는 별도 체크포인트라 이 순서에 끼어들어도 무방하다).
...
> MIT license, 오프라인 자산)를 Read해서 그 전체 내용을 `{CHART_JS_INLINE}` 자리에 그대로
> 붙여넣어 항상 인라인한다(위 7단계 참고) — 별도 자산 파일로 복사해두고 상대 경로
> `<script src>`로 참조하는 방식은 쓰지 않는다.
