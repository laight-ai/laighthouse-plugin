...
**단 한 번의 Bash 호출** 안에서 heredoc으로 `level:"ad"`로 `assets/dxd_table_rows.py`에
파이프한다 — google/meta/naver/airbridge 4개 응답을 media/airbridge로 나누는 것도, 마크다운을
파싱하는 것도 전부 스크립트가 한다:

```bash
python3 assets/dxd_table_rows.py <<'PYEOF'
{"level":"ad","d1_date":"2026-07-14","d0_date":"2026-07-15","markdown":["<google 응답 원본>","<meta 응답 원본>","<naver 응답 원본>","<airbridge 응답 원본>"]}
PYEOF
```

⚠️ **응답을 먼저 파일로 저장했다가 별도 호출로 다시 읽어서 스크립트를 실행하지 않는다** —
`echo '...'`는 이 섹션처럼 특히 큰(Naver 키워드 단위) 텍스트에서 셸 이스케이프가 깨지기
쉬운데, 그렇다고 "각 매체 원본을 파일에 저장 → 별도 호출로 다시 읽어 스크립트 실행"이라는
2단계로 가지 않는다(실제로 이렇게 하다가 툴 호출이 불필요하게 늘어난 사례가 있었다) — 위처럼
따옴표 있는 heredoc(`<<'PYEOF'`)을 쓰면 셸이 내용을 전혀 해석하지 않으므로 안전하며, 4개
응답을 받은 바로 그 Bash 호출 안에서 한 번에 끝낸다.
...
> 이건 옵션이 아니라 § 데이터 처리 원칙(절대 지침) 위반이다.** 실제 프로덕션 실행에서, Naver
> 응답이 크다는 이유로 "naver: too large, 캠페인 레벨로 집계"라고 스스로 판단한 뒤, 그
> 자리에서 광고그룹/광고 단위 실제 데이터를 가져오는 대신 **section-4에서 이미 받은 캠페인
> 단위 값과도 일치하지 않는, 출처를 알 수 없는 숫자를 만들어 `asset_group`/`ad_name`을
> `"-"`로 채운 가짜 행에 넣어 section-5에 그대로 사용한** 사고가 있었다 — 검산해보니 그
> 숫자는 section-4가 실제로 받은 같은 캠페인·같은 날짜의 값과도 전혀 달랐다. "응답이 커서
> 처리하기 부담스럽다"는 판단이 드는 순간, 다음 두 가지 중 하나만 선택지다:
> 1. **원본 마크다운 문자열을 가공 없이 전부** `markdown` 배열에 넣어 스크립트에 그대로
>    맡긴다(스크립트는 이미 대량의 행을 문제없이 처리하도록 검증돼 있다 — 크다고 사람이
>    먼저 걸러낼 필요가 없다).
> 2. 그래도 정말 처리가 불가능하다고 판단되면, 그 매체(또는 섹션 전체)를 **"데이터 준비
>    중"**으로 표시한다(§ 아래 "데이터 부족 시" 규칙).
> **다른 섹션에서 가져온 값을 재사용하거나, 비슷해 보이는 숫자를 만들어 채우는 것은 위 두
> 선택지 어디에도 해당하지 않는다** — 그 자체로 데이터 조작이며, 절대 금지 원칙 위반이다.
> section-4와 section-5는 airbridge를 제외하면 서로 다른 `group_by`(campaign vs ad)를 쓰므로
> 애초에 값을 공유할 수 있는 관계가 아니다(§ 위 "MCP 도구 호출" 절 참고) — "section-4 값을
> 재사용한다"는 발상 자체가 이 스킬의 데이터 모델과 맞지 않는다는 신호로 받아들인다.
>
> ⚠️ **위 교정 이후에도 같은 문제가 다른 형태로 재발했다 — 이번엔 "숫자 자체는 진짜"라는
> 이유로 재사용이 괜찮다고 판단한 사례였다.** 실제로 Naver `group_by:"ad"` 응답을 정상적으로
> 받아놓고도, "너무 커서 그대로 못 쓴다"고 판단해 그 실제 응답 대신 **section-4가 실제로 받은
> (진짜) 캠페인 단위 합계값**을 `asset_group`/`ad_name`을 `"-"`로 채운 행에 넣어 재사용했다.
> 이전 사고(출처 불명의 가짜 숫자)보다는 나았지만 — **숫자가 진짜인지 여부는 이 판단과 무관
> 하다.** 이미 정상적으로 받은 `group_by:"ad"` 응답이 있는데 그걸 버리고 다른 세분화 단위의
> 값으로 바꿔치기하는 것은, 그 대체 숫자의 출처가 진짜 쿼리든 상상이든 **똑같이 금지된다** —
> "진짜 숫자니까 괜찮다"는 판단 자체가 이 규칙이 막으려는 예외 만들기다. 이미 받은
> `group_by:"ad"` 마크다운 응답이 있다면 그걸 **가공 없이 그대로** `markdown` 배열에 넣는다
> (위 옵션 1) — 응답을 이미 성공적으로 받았다면 "너무 커서 못 쓴다"는 이유로 그 응답을
> 버리고 다른 것으로 바꿔치기하는 경우는 존재하지 않는다(받았으면 그대로 쓴다). 정말 응답
> 자체를 받는 데 실패했을 때만 옵션 2("데이터 준비 중")로 간다.

출력된 `[{"search":..., "html":...}, ...]` 배열(이미 D0 광고비 내림차순 정렬, ₩10,000 이하
필터, `asset_group`/`ad_name`이 빈 값이면 `-`로 치환, 캠페인 단위 airbridge 매출 공유,
`<tr>` HTML까지 완성됨)을 그대로 `{DAILY_AD_ROWS}` 자리에 넣는다. 아래 "필요 데이터"에 적힌
조인·파생지표·변화율·색상 규칙은 전부 이 스크립트가 이미 구현하고 있다 — 스크립트를 수정하지
않는 한 아래 명세를 다시 구현할 필요는 없다(section-4와 같은 asset을 `level` 파라미터로만
다르게 호출하는 것이다).

## 필요 데이터 (매체/캠페인/광고그룹/광고별, D-1/D-0 각각 별도로) — 위 스크립트의 계산 명세
