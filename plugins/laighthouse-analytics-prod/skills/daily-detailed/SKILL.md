...
6. ⚠️ **`chart.umd.min.js`(약 208KB)를 절대 자신의 응답 텍스트로 다시 타이핑(재생성)하지
   ...
   **어느 쪽이든 모델이 chart.js 바이트를 직접 다시 쓰는 일은 없어야 한다**:
   - **파일 시스템에 저장할 수 있는 호스트 (전용 "파일 복사" 도구 유무와 무관)**: 먼저
     ...
     쓰지 않는다. **없을 때만** 그 경로에 `assets/chart.umd.min.js`를 복사한다 — 전용 "파일
     ...
     "딱 한 번"뿐이다** — 그 파일이 일단 존재하면 이후의 모든 `daily-detailed`(뿐 아니라
...
> 실행"이 아니다 — 진짜 속도 개선은 배치보다 **호출 총 개수를 줄이는 것**(각 섹션 파일의 `media`
> 생략 통합 호출 참고)에서 나온다. 그래도 배치는 공짜 이득이니 계속 유지한다 — 하나씩 순차
...
호출해 매체별 일자별 광고비와 airbridge 채널별 일자별 매출을 한 번에 받고, section-4는 같은
도구를 `media` 생략 + `group_by:"campaign"`으로 1회 호출한다(google/meta/naver/airbridge 네
매체 모두 `group_by`가 같으므로 완전히 통합된다). section-5는 google/meta/naver를
`media` 생략 + `group_by:"ad"`(캠페인/광고그룹/광고 3단계가 한 응답에 다 들어있음)로 1회, 이
응답과 `group_by`가 달라 통합할 수 없는 airbridge를 `media:"airbridge"` + `group_by:"campaign"`
으로 별도 1회, 총 2회 호출한다. section-3/4/5는 각각 다루는 기간(7일 vs D-1~D0)과 `group_by`
단위가 서로 달라 응답을 섹션 간에 공유하지 않는다(각 섹션 파일의 MCP 호출 절 참고). 전부
