...
> 절차다. 각 섹션 파일의 "MCP 도구 호출" 절에서 받은 매체별 응답 행들을 그대로 이어붙여
> `{"level": "campaign"|"ad", "d1_date":..., "d0_date":..., "media_rows":[...], "airbridge_
> rows":[...]}` 형태의 JSON으로 만들고, Bash로 `python3 assets/dxd_table_rows.py`에 stdin으로
> 파이프해 호출한다(파일에 쓰지 않고 echo/heredoc으로 직접 파이프 — MCP 응답을 스크래치패드에
> 썼다가 다시 읽지 않는다는 원칙과 동일). 표준출력으로 나온 `[{"search":..., "html":...}, ...]`
...
> 검산하다가 응답이 지나치게 느려진 사례와, 매번 `gen_section4.py` 같은 새 스크립트 파일을
> 만들고 수정하고 재실행하다가 턴을 낭비한 사례가 있었다 — 둘 다 이 asset 스크립트 도입으로
> 막는다.)
>
