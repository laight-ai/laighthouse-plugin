...
>   끼워넣지 않는다.
> - 🚫 **"이 매체/섹션은 응답이 너무 크다"는 판단이 들어도, 다른 섹션·다른 날짜에서 받은 값을
>   재사용하거나 비슷해 보이는 숫자를 만들어 대신 채우지 않는다.** 실제 프로덕션 실행에서,
>   section-5(광고그룹/광고 성과)가 Naver 응답이 크다는 이유로 실제 데이터를 가져오는 대신
>   출처를 알 수 없는(다른 섹션 값과도 일치하지 않는) 수치를 만들어 넣은 사고가 있었다 — 이건
>   근사치가 아니라 명백한 데이터 조작이며, 이 지침이 금지하는 "임의 판단"의 가장 심각한
>   형태다. 응답이 크다고 느껴지면 선택지는 정확히 둘뿐이다: (1) 원본을 가공 없이 전부 담아
>   asset 스크립트(있는 섹션의 경우) 또는 필수 Bash 집계로 넘기거나, (2) 정말 처리가
>   불가능하면 그 섹션/매체를 "데이터 준비 중"으로 표시한다. 그 중간은 없다.
> - 🚫 **위 금지가 반영된 이후에도 같은 문제가 다른 모습으로 재발했다 — "대체하는 숫자가 실제
>   쿼리에서 나온 진짜 값이니 괜찮다"는 판단이었다.** section-5가 Naver `group_by:"ad"` 응답을
>   정상적으로 받아놓고도 "너무 커서 못 쓴다"며 그 응답 대신 다른 섹션(section-4)이 실제로
>   받은 캠페인 단위 진짜 합계값으로 바꿔치기한 사례다. **대체 숫자가 진짜 쿼리 결과인지
>   여부는 이 규칙과 무관하다** — 이미 정상적으로 받은 응답이 있다면 그 세분화 단위 그대로
>   쓴다(위 옵션 1). "받았지만 크다"는 이유로 다른 세분화 단위의 값으로 바꾸는 경우는 존재하지
>   않는다 — 응답을 못 받았을 때만 옵션 2("데이터 준비 중")로 간다.
...
> 조합본을 별도 파일로 만들었다가 나중에 다시 읽어서 합치는 방식은 절대 금지한다 (실제
> 프로덕션 실행에서 이 방식으로 섹션마다 파일을 만들다가 최종 저장/완료 메시지 전송 전에
> 툴호출 제한에 걸려 사용자에게 아무 결과도 보여주지 못한 사례가 있었다). 각 섹션의 HTML
> 문자열은 그 섹션을 만드는 같은 턴에서 바로 최종 문서 조합에 이어 쓴다 — 파일을 매개로 왕복하지
> 않는다. **최종 저장 시점에 딱 한 번**, 완성된 전체 HTML을 위 "실행 순서"에 정해진 저장 경로에
> Write한다.
>
> 위 금지 원칙은 **재사용 가능한 파이프라인 파일을 그 자리에서 새로 만들지 말라는 것**이다 —
> 이미 이 스킬 폴더에 존재하는(모델이 실행 중에 만든 게 아닌) 검증된 asset 스크립트를 그대로
> 호출하는 것은 이 금지에 해당하지 않는다(§ 아래 `assets/dxd_table_rows.py` 참고, `chart.
> umd.min.js`를 파일로 두고 그대로 인라인하는 것과 같은 이유).
>
> **section-4/5(`group_by`가 `campaign`/`ad`인 응답)의 D-1 vs D-0 조인·6개 파생지표(CTR/CPA/
> ROAS 등)·변화율·화살표·색상·₩10,000 필터·정렬·`<tr>` HTML 생성은 전부 `assets/
> dxd_table_rows.py`를 호출해서 처리한다 — 손으로(사고 과정에서 프로즈로 한 행씩 검산하며)
> 계산하지 않고, 그때그때 새 스크립트를 짜지도 않는다.** 이건 선택 가능한 옵션이 아니라 필수
> 절차다. `get_ad_performance_daily_table`은 **JSON 행 배열이 아니라 마크다운 표(파이프 `|`
> 텍스트) 문자열**을 반환한다 — 그 원본 문자열을 손으로 JSON으로 옮겨 적거나(전사 실수·"이
> 행은 안 중요해 보이니 빼자" 식의 임의 선별 위험 — 실제로 이렇게 하다가 ₩10,000 초과 행이
> 조용히 누락될 뻔한 사례가 있었다) 그걸 파싱하는 별도 스크립트 파일을 새로 만들지 않는다 —
> 각 섹션 파일의 "MCP 도구 호출" 절에서 받은 응답 문자열들을 **가공 없이 원본 그대로**
> `{"level": "campaign"|"ad", "d1_date":..., "d0_date":..., "markdown":["<응답1 원본>",
> "<응답2 원본>", ...]}` 형태로 감싸고, Bash로 **단 한 번의 호출** 안에서 따옴표 있는
> heredoc(`python3 assets/dxd_table_rows.py <<'PYEOF' ... PYEOF`)으로 stdin에 파이프한다 —
> `echo '...'`는 크고 줄바꿈·따옴표가 많은 마크다운 응답에서 셸 이스케이프가 깨지기 쉬우므로
> 쓰지 않는다. **응답을 먼저 파일에 저장했다가 별도 호출로 다시 읽어서 스크립트를 실행하는
> 2단계로 절대 가지 않는다** — 실제로 이렇게 하다가(파일 준비 → 원본 저장 → 별도로 스크립트
> 실행) 툴 호출이 불필요하게 늘어난 사례가 있었다. MCP 응답을 스크래치패드에 썼다가 다시
> 읽지 않는다는 원칙과 동일하게, 응답을 받은 바로 그 Bash 호출 안에서 한 번에 끝낸다. 마크다운
> 파싱·media별 분리·조인·계산·정렬·필터·HTML
> 생성을 전부 스크립트가 처리한다. 표준출력으로 나온 `[{"search":..., "html":...}, ...]`
> 배열을 그대로 `{DAILY_CAMPAIGN_ROWS}`/`{DAILY_AD_ROWS}` 자리에 넣는다 — 이미 D0 광고비
> 내림차순 정렬과 ₩10,000 이하 필터가 적용된 상태이고, `<tr>` 마크업도 각 섹션 파일의 HTML
> 절과 동일한 구조로 이미 완성되어 있으므로 추가 가공이 필요 없다.
> (실제 프로덕션 실행에서, 이 계산을 스크립트 없이 사고 과정에서 캠페인/광고 한 줄씩 프로즈로
> 검산하다가 응답이 지나치게 느려진 사례, 매번 `gen_section4.py` 같은 새 스크립트 파일을
> 만들고 수정하고 재실행하다가 턴을 낭비한 사례, 그리고 asset 스크립트 도입 이후에도 그
> 입력 형식(JSON)과 MCP 응답의 실제 형식(마크다운 문자열)이 다르다는 것을 깨닫지 못해 마크다운
> →JSON 변환용 파서 스크립트를 별도로 만들거나 응답을 손으로 골라 옮겨 적은 사례까지 총 세
> 가지가 있었다 — `assets/dxd_table_rows.py`가 마크다운 문자열을 직접 받아 파싱하도록
> 확장해서 셋 다 막는다.)
>
> `group_by`가 `campaign`/`ad`/`ad-set`이지만 이 asset 스크립트의 입력 형식과 맞지 않는 다른
> 섹션(다른 report_type 등)에서는 여전히 기존 방식대로 Bash(grep/awk/jq 등, 파일로 남기지 않는
> 즉석 명령)로 전체 행에 대해 정확하게 필요한 매체 필터링·집계·정렬을 수행한다 — 이것도 선택
> 가능한 옵션이 아니라 필수 절차다. 그 결과로 나온 작은 요약표만 컨텍스트에 남긴다 — 원본
> 테이블 전체를 손으로 옮겨 적거나 머릿속으로 합산하지 않는다. "이 정도만 훑어보고 나머지는
> 추정" 같은 부분 처리는 절대 금지한다. 만약 집계를 시도하다가 effort/시간이 부족하다고
> 느껴지면, **눈으로 훑어서 대략적인 결과로 넘어가지 말고**, 더 작은 단위로 나눠서(예: 날짜별로
> 쪼개서 합산한 뒤 합치기) 집계를 끝까지 완료한다. 정확한 계산 없이 만들어낸 순위·합계·TOP-N은
> 이 보고서에 절대 포함시키지 않는다 — 차라리 그 섹션을 "데이터 준비 중"으로 표시하는 것이
> 근사치를 보여주는 것보다 낫다.
...
3. ⏱ **필수 체크포인트 — 2단계 응답을 받은 즉시, 다음 단계로 넘어가기 전에 스켈레톤을 먼저
   ...
   — 실제 프로덕션 실행에서 이 순서(스켈레톤 게시 없이 전체를 끝까지 만들다가 저장 직전에
   제한에 걸림)로 인한 실패가 있었다. 스켈레톤 게시는 선택 사항이 아니라 이 순서상 반드시
   거쳐야 하는 단계다.
...
> ℹ️ **이 스킬을 실행하는 호스트의 Bash 도구 기본 셸은 `sh`(dash)일 수 있고 `bash`가 아닐 수
> 있다.** `diff <(cmd1) <(cmd2)` 같은 **프로세스 치환(`<(...)`)은 bash 전용 문법**이라 `sh`에서
> `Syntax error: "(" unexpected`로 실패한다(실제로 `report-scaffold.html`을 SKILL.md 임베디드
> 버전과 비교할 때 이 에러가 난 적이 있다). 이런 비교·조합 작업이 필요하면 처음부터
> `bash -c '...'`로 감싸서 실행하거나, 프로세스 치환 없이(예: 중간 결과를 변수/파일에 먼저
> 담아 일반 `diff file1 file2`로) 실행한다 — 실패 후 재시도로 턴을 낭비하지 않는다.
...
> 실패하고 모든 차트가 깨진 사례가 있었다 — 위 7단계 참고). **모델이 그 208KB를 직접 응답
