...
> "<응답2 원본>", ...]}` 형태로 감싸고, Bash로 `python3 assets/dxd_table_rows.py`에 stdin으로
> 파이프해 호출한다(파일에 쓰지 않고 echo/heredoc으로 직접 파이프 — MCP 응답을 스크래치패드에
> 썼다가 다시 읽지 않는다는 원칙과 동일). 마크다운 파싱·media별 분리·조인·계산·정렬·필터·HTML
