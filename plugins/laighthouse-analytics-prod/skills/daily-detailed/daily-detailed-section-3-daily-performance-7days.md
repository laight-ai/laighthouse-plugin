...
**report_type:** `daily-detailed` — **브리즘(airbridge 기반) 전용** (항상 포함). 기준일을
...
- ⚠️ 이 응답은 **section-4(캠페인 성과)/section-5(광고그룹 및 광고 성과)와 공유되지 않는다** —
  section-4/5는 D-1~D0 이틀만 다루고 각각 `group_by:"campaign"`/`group_by:"ad"`가 필요해
  이 섹션의 `group_by:"media"` 응답으로는 대체할 수 없다(이 섹션이 필요로 하는 범위가 section-4/5
  범위(이틀)를 포함하는 방향이라 하더라도, group_by 단위 자체가 달라 재사용이 불가능하다).
  section-4/5는 각자 별도로 호출한다.
...
각 날짜(총 7일)에 대해:
- `광고비` = 공유 응답에서 `media`가 google/meta/naver인 세 행의 해당 날짜 `cost` 합
- `매출` = 공유 응답에서 `media`가 airbridge인 행 중 해당 날짜 광고 채널(`Google Ads`/`Meta Ads`/`Naver Ads`) 행
  `airbridge_revenue` 합
...
2. **`labels` 배열의 7일은 항상 연속된 달력 날짜**이므로, 인덱스는 "찾기"가 아니라 **날짜
   ...
   - 예시: 기준일이 8/4이면 `labels = [7/29, 7/30, 7/31, 8/1, 8/2, 8/3, 8/4]`. 프로모션이
     "7/21~8/31"이면 `raw_start_idx = 7/21 − 7/29 = -8`(음수 → `start_idx=0`),
     `raw_end_idx = 8/31 − 7/29 = 33`(6 초과 → `end_idx=6`) → 이 프로모션은 **7일 전체
     (인덱스 0~6)**를 덮는다. 프로모션이 "6/29~7/29"이면 `raw_start_idx = 6/29-7/29 = -30`
     (→0), `raw_end_idx = 7/29-7/29 = 0`(그대로) → **인덱스 0(7/29) 하나만** 덮는다.
...
4. `range_label`은 `"{시작월}/{시작일}~{종료월}/{종료일}"` 형식으로 만든다 (같은 달이면 뒤쪽
   ...
   그대로 쓴다** — 클램핑된 `start_idx`/`end_idx`에 대응하는 날짜로 바꿔 쓰지 않는다(라벨은
   프로모션의 실제 기간을 보여주는 것이고, 클램핑은 오직 차트 위에 그릴 위치 계산에만
   쓰인다).
...
(function(){
  ...
  if (wrap && promos && promos.length) {
    ...
    promos.forEach((p, row) => {
      // 방어적 clamp — 데이터 가공 단계에서 이미 0~6으로 잘렸어야 하지만, 혹시
      // 잘못된 값이 들어와도 차트 밖으로 브래킷이 그려지지 않도록 한 번 더 보정한다.
      const s = Math.max(0, Math.min(n - 1, p.start_idx));
      const e = Math.max(0, Math.min(n - 1, p.end_idx));
      const xStart = leftEdge(s);
      const xEnd = rightEdge(e);
...
- **ROAS 축(우측, y2)은 항상 0%부터 시작한다**(`min:0`) — 최고점(`max`)은 지정하지 않아
  데이터 실황에 따라 자동으로 조정된다. `min`을 지정하지 않으면 ROAS 값이 전부 100% 이상인
  경우 축이 100%쯤부터 시작해서 선이 잘려 보이는 것처럼 보이는 문제가 생길 수 있다.
...
- 브래킷 좌우 끝은 **날짜 영역(밴드) 전체**에 맞춘다 — 카테고리 중심점이 아니라 밴드 폭의
  절반만큼 보정한 좌우 경계를 쓴다 (위 Script의 `leftEdge`/`rightEdge` 참고). **`start_idx`/
  `end_idx`는 "데이터 가공" 단계에서 이미 0~6으로 클램프됐어야 하지만, Script에서도 한 번 더
  `Math.max(0, Math.min(n-1, ...))`로 방어적으로 clamp한다** — 프로모션 기간이 실제 차트
  범위와 안 맞게(예: 전체 7일을 덮어야 할 프로모션이 중간 며칠만 덮은 것처럼) 그려지는
  문제가 생길 수 있다. 특히 "labels 배열에서 인덱스를 찾는다"처럼 애매하게만 지시하면 정확한
  날짜 차이 계산 없이 잘못 구현되기 쉽다 — 위 "데이터 가공" 2단계의 명시적 날짜 차이 공식을
  그대로 따른다.
