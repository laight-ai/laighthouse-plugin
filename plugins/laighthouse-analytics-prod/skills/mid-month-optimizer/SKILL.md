...
3. **20% 초과 증액 게이트 (증액이 있을 때만)**: 조정안 중 기존(base_value) 대비 20%를
   ...
   - **히스토리 분기**: 사용자가 반영되지 않은 히스토리를 제공하면(대화 답변이든,
     ...
     반영해 2단계(조정안 산출)부터 재산출한 뒤 다시 진행한다. 이때의
     `stage="aborted"`는 중단이 아니라 재산출 신호다.
   - 판단 근거에 동의하고 히스토리가 없을 때만 이후 호출에 `over_change_confirmed=true`를
     더하고, 거절하면 `over_change_confirmed=false`로 호출하거나 해당 매체 조정안을
     재산출해 2단계부터 다시 진행한다.
...
- **20%를 초과하는 증액은 증액 매체가 있을 때만 서버가 확인을 요구한다** — 20%를 초과하는 증액 확인은 위 플로우 3단계의 2차 조기 호출로 서버 게이트에 맡긴다 — LLM이 질문을 대필하지 말고 서버가 돌려준 질문 문구를 그대로 중계한다 (질문에 자신의 판단 근거(rationale)와 히스토리 질문이 이미 포함되어 있으므로 별도로 재작성하지 않는다). 2차 호출에서 어떤 매체든
  ...
  `over_change_confirmed`를 설정하지 말고 그 히스토리를 반영해 조정안을 재산출한다 —
  이때 서버가 돌려주는 `stage="aborted"`는 중단이 아니라 재산출 신호다.
  **사용자에게 보여주고 동의받기 전에는
