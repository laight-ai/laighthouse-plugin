...
- **② 매체별 사유 (2차 호출, 해당 매체가 있을 때만)**: 툴이 근거를 보고 확인이 필요하다고
  판단한 매체에 대한 질문으로, **툴이 돌려준 질문 문구를 그대로 쓴다** — 무엇을 묻는
  질문인지 이 스킬이 다시 풀어 쓰지 않는다. 게이트 질문은 **한 번의 응답에 모두 담겨
  온다** — 전부 한 메시지로 물은 뒤 모든 답을 모아 정확히 1회만 재호출한다(매체당 재호출
  금지). 사유 유무가 조정 폭을 바꾼다.
...
1. **1차 호출**: `optimize_mid_month_budget(brand_name=...)` — `decisions` 없이.
   - `stage="needs_user_input"`이면 `questions`의 **① 효율 전망** 질문을 문구 그대로 물은 뒤,
     ...
     - `outlook_media` — 사용자가 특정 매체만 지목한 경우 그 매체명 목록. 이 시점에는 아직
       `budget_reference`가 없으므로, 쓸 수 있는 매체명은
       **`questions[0].options["outlook_media"]` 안내에 실려 온다** — 그 목록의 이름과 정확히
       일치시킨다. **전체이거나 언급이 없으면 생략한다** (생략 = 전체 매체).
...
3. **2차 호출 (저장 없음)**: `decisions=[...]` + 1차에서 받은 답변 인자
   ...
   - **매체별 사유 질문(②)이 돌아오는 경우**: `stage="needs_user_input"`으로 게이트 대상
     매체의 질문이 **한 번의 응답에 모두 담겨** 온다(`questions[]` 한 항목이 한 매체 —
     그 항목의 `arguments`가 `decisions[<매체>].over_change_reason`). 모든 질문을 **문구
     그대로** 한 메시지에 전부 물은 뒤, 받은 답을 각각 해당 매체의 `decisions` 항목에
     `over_change_reason`으로 담아 **정확히 1회만** 재호출한다 — 매체당 하나씩 나눠
     재호출하지 않는다.
...
5. **승인 후 저장**: 3단계 응답의 `questions[0]`(`arguments`가 `["approved"]`)에 승인 질문
   문구가 담겨 온다 — 표와 요약을 보여준 뒤 그 **`questions[0].question`을 문구 그대로**
   물어 동의를 받는다(동의 문구를 직접 작문하지 않는다). 채팅에서 명시적 동의를 받은 뒤에만
...
- **월 전망 경고는 차단이 아니라 전달 대상이다**: 월 전망이 월 예산을 넘거나(총액 고정
  ...
  숨기지 않는다.
- **고정 규칙으로 유지되는 매체**: 1차 응답의 `context.held_media`에 담겨 오는 매체는 툴의
  고정 규칙에 따라 기존 금액(base)을 그대로 유지한다 — 증감을 제안하지 말고, 고정 규칙 때문에
  유지된다는 사실을 사용자에게 그대로 전달한다. 다른 매체와 마찬가지로 `decisions`에 포함하고
  `rationale`에는 '유지'라고 적는다. 어떤 매체가 해당되는지는 **툴 응답이 유일한 출처다** —
  매체명을 외워 두거나 추측하지 않는다.
