...
— 행은 디스커버리로 받은 `media_list`의 각 매체 + (`has_organic`이고 매출 있음 모드면) Organic
한 행. 고정 매체 목록·"Others" 행은 없다.
...
section-3이 1회 호출한 `get_ad_performance`(`filters`·`metrics` 생략, `time_grain:"month"`,
...
- `cost`/`impression`/`click`/`revenue`/`conversion`: 해당 매체 행의 해당 월 역할 키 값 — 한 행에
  전부 들어있다(별도 조인 없음). `metric_keys`에 없는 역할(revenue/conversion)은 행에도 넣지 않는다.
...
"s5": { "rows": [
  { "name": "<media 값 1>",
    "m1": { "cost": 1000000, "impression": 520000, "click": 8300, "revenue": 50000000, "conversion": 120 },
    "m0": { "cost": 1100000, "impression": 501000, "click": 7900, "revenue": 52000000, "conversion": 130 } },
...
%(M-1이 0/`-`이면 미표시), ROAS·CTR 변화는 %p; 화살표(▲/▼)는 원본 부호 기준, 반올림 표시값이
0.0이면 화살표 없이 검정; **모든 지표 증가=빨강(#dc2626)/감소=파랑(#2563eb)**; 행은 받은 순서대로
렌더링; `conversion` 역할이 없으면 전환 컬럼 생략; 매출 없음 모드면 Organic 행 제외.
