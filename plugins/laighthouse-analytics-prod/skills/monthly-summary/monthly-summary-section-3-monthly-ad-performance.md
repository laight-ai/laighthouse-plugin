...
- **`media` 파라미터를 생략한다** — 생략하면 이 도구는 google/meta/naver/airbridge(및 이
  ...
  - `media`가 정확히 `"google"`/`"meta"`/`"naver"`인 행 — 매체당 월별로 **이미 합산된 한
    줄씩**이며, 이 행들의 `cost`가 예전에 `group_by:"total"`로 받던 값과 동일하다.
...
- **이 호출의 응답은 `monthly-summary-section-4-revenue-trend.md`(section-4)와
  `monthly-summary-section-5-channel-performance.md`(section-5)가 그대로 재사용한다** — 세
  섹션이 각자 호출하지 않는다. section-4는 이 응답의 airbridge 행 전체를, section-5는 이
  응답 중 전월(M-1)·당월(M0) 두 달치 행만 가져다 쓴다(section-5가 필요로 하는 "전월~당월"
  범위는 이 섹션의 "5개월 전~당월" 범위 안에 항상 완전히 포함된다).
...
```javascript
// Breezm Executive Monthly Section 3: 월별 광고 성과 혼합 차트
(function(){
