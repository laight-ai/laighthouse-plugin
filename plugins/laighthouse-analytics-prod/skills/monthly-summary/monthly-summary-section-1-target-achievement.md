...
(`mtd`/`daily`/`executive-daily`와 동일한 데이터·계산 로직 — `target_date`가 월말이면 그
달의 최종 진행률이, 월중이면 그 시점까지의 진행률이 자연스럽게 나온다). 모든 "매출"은
Airbridge 매출(`airbridge_revenue`), 광고 채널은 airbridge 응답 `channel` ∈ {`Google Ads`,
`Meta Ads`, `Naver Ads`} 행이다 (SKILL.md 공통 규칙 참고).
...
이 세 응답은 이 섹션에서만 쓴다 — `executive-monthly` 보고서에는 아직 이 응답을 공유할 다른
...
메시지)에서 동시에** 발사한다 — 이 호출 하나가 매출 실적과 fallback 소진액 후보를 항상 함께
가져오므로 조건 분기를 기다릴 필요가 없다 (SKILL.md 「병렬 호출 지침」 참고). 이 응답은 이
섹션에서만 쓴다 — 현재 `executive-monthly` 보고서에는 이 응답을 공유할 다른 섹션이 없다
