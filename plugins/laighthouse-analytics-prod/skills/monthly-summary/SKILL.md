...
MCP 데이터를 받아 **라이트하우스 스타일 Executive 월간 보고서**(HTML)로 렌더링한다. **대상
...
`creative-detailed`/`creative-summary`). 이 스킬은 호출되면 항상 Executive 월간 보고서를
...
>   ROAS·변화량·화살표·색상·정렬과 6개월 라벨·각주도 전부 빌더가 계산한다 — 손계산하지 않는다.
...
4. **계산**: section-1 값 판정, section-3 6개월 배열 3개, section-4 6개월 배열 2개, section-5
   5개 항목의 M-1/M0 원시 수치 — 각 섹션 파일의 매핑 규칙대로 빌더 입력 JSON 값을 만든다
   (포맷팅·파생지표·정렬은 빌더 몫이다).
5. **section-2 Executive Summary 작성** — 신규 MCP 호출 없이(2단계의 `list_promotions` 포함)
...
6. **최종 빌드**: `assets/build_report.py`에 값 JSON을 heredoc으로 넘겨 최종 HTML을 생성한다.
   출력 경로(`out`)는 `~/Downloads/laighthouse-reports/브리즘_monthly-summary_{기준_일자}.html`
   (디렉터리는 빌더가 만든다).
...
> 호출은 한 메시지 안에서 동시에(병렬 tool call로) 발사한다** — 이 스킬은 6개 호출 전부가
> 무의존이라 한 배치로 끝난다. 배치의 실제 효과는 "턴 오버헤드 제거"다(네트워크 동시 실행
> 보장은 아님 — 실측 daily-summary 참고). 진짜 속도는 (a) 호출 총 개수 축소(`media` 생략
> 통합으로 최대 17회 → 6회), (b) asset 스크립트(재타이핑·손계산 제거)에서 나온다.
