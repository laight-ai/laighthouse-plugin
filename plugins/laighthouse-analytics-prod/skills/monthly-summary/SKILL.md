...
description: >
  ...
  전용 Executive 월간 보고서 생성 스킬. "임원용 월간 보고서", "executive monthly" 요청 시 사용.
  대상 월(또는 그 시점까지)의 목표 대비 진행 상황을 임원 관점으로 보여주는 월간 보고서. 대상 브랜드는 브리즘 하나뿐이다.
metadata:
  version: "1.0.0"
---
...
브리즘(airbridge 기반) 하나뿐이다.** 이 스킬은 **Executive 월간 보고서**(대상 월(또는 그 시점까지)의 목표 대비 진행 상황을 임원 관점으로 보여주는 월간 보고서) 전용이며,
...
다른 보고서 종류를 선택하는 개념은 없다 — 호출되면 항상 Executive 월간 보고서를 렌더링한다.
...
> **섹션 HTML을 조합하는 단계에도 동일하게 적용된다** — `section2.html`,
> `section4_rows.json`, `final_report.html`(임시 스테이징본) 처럼 섹션별 조각이나 중간
> 조합본을 별도 파일로 만들었다가 나중에 다시 읽어서 합치는 방식은 절대 금지한다 (sibling 스킬인
...
> 매개로 왕복하지 않는다. **최종 저장 시점에 딱 한 번**, 완성된 전체 HTML을 위 "실행 순서"에
> 정해진 저장 경로에 Write한다.
...
| 파라미터 | 설명 | 예시 |
|--------|------|------|
| 보고서 제목 | 보고서 상단 타이틀 | 브리즘 Executive 월간 보고서 |
...
1. 파라미터를 파싱한다. report_type은 `monthly-summary`로 고정되어 있다.
2. target/achievement 수치를 호출한다 — `mcp__laighthouse__get_target_progress_v2`를
   ...
   **google/meta/naver 세 번**(`media`만 바꿔) 호출한다
   (`monthly-summary-section-1-target-achievement.md` 참고). 세 매체 모두
   ...
   N/A로 표시하고, 대체 값은 아래 4단계에서 **이미 받아둔**
   `get_ad_performance_monthly_table`(`media` 생략, `group_by:"media"`, `start_month`=`end_month`=당월,
   `day_offset`=target_date.day) 응답 중 해당 매체 행의 `cost`를 그대로 쓴다 — 목표 판정 결과를
   기다렸다가 추가로 호출하지 않는다(섹션 파일의 대체 규칙 참고). 매출 실적(`기간 매출`/`광고
   매출`)도 목표 유무와 무관하게 **항상** 이 4단계 호출의 airbridge 행에서 가져온다 —
...
3. ⏱ **필수 체크포인트 — 2단계 응답을 받은 즉시, 다음 단계로 넘어가기 전에 스켈레톤을 먼저
   게시한다.** 나머지 4개 섹션(Executive Summary/월별 광고 성과/매출 추이/매체 성과 비교)은
   전부 "데이터 준비 중" placeholder(§ 데이터 부족 시 규칙과 동일한 마크업)로 채운 전체 골격을
   1차로 Artifact(또는 `mcp__visualize__show_widget`)에 게시한다 — 이후 각 섹션 데이터가
   준비되는 대로 같은 파일을 갱신(재게시)해 placeholder를 실제 값으로 교체한다. **이 체크포인트를
   생략하고 나머지 단계를 전부 마친 뒤에야 한꺼번에 결과를 내놓으려다가 끝에서 툴호출 예산이
   부족해지면, 사용자는 아무 결과도 못 보고 끝난다** — sibling 스킬 `daily-detailed`의 실제
   프로덕션 실행에서 이 순서(스켈레톤 게시 없이 전체를 끝까지 만들다가 저장 직전에 제한에
   걸림)로 인한 실패가 있었다. 스켈레톤 게시는 선택 사항이 아니라 이 순서상 반드시 거쳐야 하는
   단계다.
4. 나머지 `mcp__laighthouse__*` generic 도구를 호출해 각 섹션 수치 데이터를 가져온다 (각 섹션
   ...
   - ⚠️ **이 두 도구를 부를 때 `media` 파라미터를 아예 생략한다** (섹션 파일에 다르게 적혀
     ...
     함께 온다 — 매체별로 나눠 여러 번 부르지 않는다. section-1의 매출 실적+fallback 소진액용
     호출(당월 1개월), section-3의 월별 광고 성과용 호출(최근 6개월, section-4/5가 공유
     재사용)이 각각 이렇게 media 생략 1회 호출로 통합되어 있다(각 섹션 파일 참고) — 이 스킬의
     데이터 호출이 `get_target_progress_v2` 3회 + `get_ad_performance_monthly_table` 2회(둘 다
     media 생략) + `list_promotions` 1회, 총 **6회**로 줄어드는 이유가 이것이다(예전에는
     17회 이상).
...
7. ⚠️ **`chart.umd.min.js`(약 208KB)는 항상 실제 HTML의 `{CHART_JS_INLINE}` 자리에 전체
...
8. 아래 **보고서 골격**에 섹션들을 삽입해 렌더링한다. 완성된 HTML은 호스트가 지원하는 경로
   ...
   - **파일로 저장** (모든 호스트, 항상): 최종 HTML을 `~/Downloads/laighthouse-reports/브리즘_
     monthly-summary_{기준_일자}.html` 경로에 그대로 저장한다 (디렉터리가 없으면 새로 만든다).
     파일명 예: `브리즘_monthly-summary_2026-05-15.html`.
...
> 클라이언트 구현에 달려 있다"고 명시한다(`daily-summary`에서 실측으로 확인된 사실이며,
> `get_ad_performance_monthly_table`을 쓰는 이 스킬도 같은 MCP 서버를 거치므로 동일하게
> 적용된다고 본다). 배치의 실제 효과는 **"턴 오버헤드 제거"**이지 "네트워크 동시 실행"이
> 아니다 — 진짜 속도 개선은 배치보다 **호출 총 개수를 줄이는 것**(아래 6개 통합 호출)에서
> 나온다. 그래도 배치는 공짜 이득이니 계속 유지한다 — 하나씩 순차 호출하면 호출마다 브랜드
> 권한 확인 왕복과 턴 지연이 누적되어 보고서 생성이 느려진다.

- ⚠️ **이 스킬 전체가 조건부 2차 라운드 없이 데이터 호출 6개를 전부 한 메시지에서 동시에
  발사한다** — section-1의 매출 실적+fallback 소진액 호출이 `media` 생략(전체 매체 반환)으로
  바뀌면서, 목표 판정 결과를 기다릴 이유 자체가 없어졌다. 아래 6개를 처음부터 전부 한
  메시지에 담아 발사한다:
  1. `get_target_progress_v2` × 3 (media=google/meta/naver)
  2. `get_ad_performance_monthly_table` × 1 (`media` 생략, `group_by:"media"`, 당월 1개월,
     `day_offset`=target_date.day) — section-1의 매출 실적 + fallback 소진액 후보
  3. `get_ad_performance_monthly_table` × 1 (`media` 생략, `group_by:"media"`, 5개월 전 ~
     당월, `day_offset`=target_date.day) — section-3/4/5 공유
  4. `list_promotions` × 1 (당월 1일 30일 전 ~ target_date) — section-2 전용(공유할 다른
     섹션 없음)
- 결과적으로 이 스킬의 데이터 호출은 **총 6회**(모두 한 배치)로 끝난다 — 예전에는 section-1의
  조건부 fallback(최대 3회) + section-3(4회) + section-4(1회) + section-5(4회)까지 포함하면
  최대 17회 이상이었다.
...
```
브리즘 Executive 월간 보고서({기준_일자}) 생성 완료.
...
예시:
```
브리즘 Executive 월간 보고서(2026-05-15) 생성 완료.
가장 인상적인 부분: Naver Ads ROAS가 5,036.7%로 세 매체 중 가장 두드러졌습니다.
— by LaightAI
📁 C:\Users\minhyeok\Downloads\laighthouse-reports\브리즘_monthly-summary_2026-05-15.html
```
...
| 순서 | 섹션 | Import 경로 |
|-----|------|------------|
| 1 | 목표 달성 현황 | `@import monthly-summary-section-1-target-achievement.md` |
| 2 | Executive Summary | `@import monthly-summary-section-2-executive-summary.md` |
| 3 | 월별 광고 성과 | `@import monthly-summary-section-3-monthly-ad-performance.md` |
| 4 | 매출 추이 (6개월) | `@import monthly-summary-section-4-revenue-trend.md` |
| 5 | 매체 성과 비교 (M-1 vs M0) | `@import monthly-summary-section-5-channel-performance.md` |

section-1은 `mtd-detailed`/`daily-detailed`/`daily-summary`의 section-1과 **완전히 동일한 데이터·계산
로직**이다 — 목표 대비 소진율/매출 달성률/ROAS, no-budget/부분 목표(cost만 있고 revenue는
N/A) 처리 규칙도 동일하다. 다른 report_type과 달리 이 섹션은 "월초~기준일"이 아니라 **대상
월 전체(또는 target_date까지)**를 다룬다는 개념적 차이가 있지만, 계산식 자체(`month`=대상 월,
`as_of_date`=target_date)는 동일하므로 target_date를 월말로 주면 자연히 월 전체 결과가 나온다.

section-3(월별 광고 성과)은 `mtd-summary`의 section-3과 **완전히 동일한 데이터·차트
로직**(광고비/매출/ROAS 혼합 차트, 최근 6개월, `day_offset` 필수, tooltip ₩/% 표기, 2줄 각주)
이다.

section-4(매출 추이)는 `mtd-summary`의 section-4와 데이터·차트 로직(전체 매출/광고 매출
라인, 최근 6개월, `day_offset` 필수, 3줄 각주)이 동일하되, **두 번째 각주(정의 문구)의 문구가
다르다** — `mtd-summary`는 "'전체 매출'은 광고 매출과 자연 구매 매출을 합친 전체 매출을
의미합니다"이지만, 이 섹션은 "'전체 매출'은 오거닉 매출과 광고 매출, 기타 매출(CRM, 제휴
마케팅 등)을 합친 전체 매출을 의미합니다"로 **"기타 매출(CRM, 제휴 마케팅 등)"이 추가**돼
있다 — 사용자가 의도된 문구임을 확인함(2026-07-31).

section-5(매체 성과 비교, M-1 vs M0)는 `daily-summary`의 section-5(매체별 성과, D-1 vs D-0)
와 동일한 비교 단위·지표 세트를 쓴다 — **Naver Ads/Google Ads/Meta Ads/Organic/Others 5개
항목 × 광고비/매출/예약 완료/ROAS 4개 지표**를 M-1(전월) vs M0(당월)으로 비교한다. `day_offset`
으로 전월을 당월과 동일 일자까지 자른 동기 비교라는 점, 헤더 구조(지표명 colspan=2 + 월 표기,
전부 검정·중앙 정렬), 변화량 표기(괄호+색상, 네 지표 전부 증가=긍정)는 `daily-summary`의
section-5와 동일하다. `mtd-summary`의 section-5(매체 성과 비교, 전월 vs 당월)도
같은 형식이다 — 세 report_type의 매체 비교 섹션이 동일한 포맷을 공유한다. "Others는 CRM, 제휴 마케팅 등 기타 채널을 의미합니다" 각주로 Others의 정의를
명시한다(section-4의 "기타 매출" 정의와 맥이 통한다).

section-2(Executive Summary)는 `daily-summary`의 section-2와 목적·HTML 골격은 같지만
비교 기준이 다르다 — **"전일 대비"가 아니라 "전월 대비"(M-1 vs M0, section-5 데이터
재사용)**이며, `list_promotions` 룩백도 7일이 아니라 **30일**(`mtd-summary`와 동일)을 쓴다.
**신규 요구사항으로 "분기 단위 장기 추이" 비교가 추가됐다** — 이번 분기 경과분(예: 8월이면
7~8월)과 전분기 동일 개월 수(4~5월)를 비교하는 항목이며, section-3/4가 이미 받은 "최근
6개월" 데이터만으로 계산 가능해 새 MCP 호출이 필요 없다(최악의 경우에도 필요 개월 수가
6개월을 넘지 않음을 확인함). 사용자가 제공한 작성 원칙에 "직전 7일 이내"와 "직전 30일 내"가
섞여 있던 모순을 30일로 통일해서 해소했다(자세한 내용은
`monthly-summary-section-2-executive-summary.md` 참고).
...
> 7단계 참고) — 상대 경로 `<script src="chart.umd.min.js">`로 자산을 따로 두는 방식은 쓰지
