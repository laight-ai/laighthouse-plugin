...
1. **소재 특이사항**: section-5의 소재 단위 성과에서 눈에 띄는 사항 — 특히 두 개 이상 지표의
   역상관에 주목한다 (예: CTR은 상대적으로 우수한데 ROAS는 낮은 경우 — 매출 없음 모드라면
   CTR은 높은데 CPC도 높은 경우, 클릭은 많은데 전환이 적은 경우 등).
...
**매출 없음 모드**(`metric_keys`에 revenue 없음): 분석 지표는 CTR·CPC·클릭(·전환이 있으면
전환·CPA)뿐이다. **매출·ROAS·매출 효율을 언급하지 않는다**(데이터가 없다). CPC 등 금액에는
`discover.py`의 `currency`를 붙인다.
