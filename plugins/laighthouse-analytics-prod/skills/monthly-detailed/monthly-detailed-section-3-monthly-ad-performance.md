...
**report_type:** `monthly-detailed` (항상 포함). 최근 6개월(당월 포함), 연-월 단위.
...
- **`filters`를 생략한다** — 이 1회 호출로 월별·매체별(`media_list`의 각 값) 행에 더해,
  `has_organic`이면 `media`가 `null`인 행(Organic — 광고비 없이 매출만 귀속)도 함께 받는다.
  각 행에 `month`("YYYY-MM") 키와 cost/impression/click/revenue(/conversion) 역할 키 등 지표가
  함께 들어있다(별도 매출 응답 없음).
...
| `revenue` | 월별: 매체 행(`media` non-null)의 revenue 키 합 — 광고 성과 차트이므로 Organic 행은 제외 |
...
- **6개월 전부 넣는다** — 행이 없는 월(데이터 적재가 늦게 시작된 경우 포함)도 labels에서
  빼지 말고 0으로 채운다. 광고비만 있고 매출이 없는 월은 매출/ROAS만 0으로 두고 광고비는
  실제 값 그대로 (추정/보간 금지).
