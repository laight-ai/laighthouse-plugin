...
> 옵션이 아니라 필수 절차다. 매체별 응답 행을 그대로 이어붙여 `{"m1_month":..., "m0_month":...,
> "media_rows":[...], "airbridge_rows":[...]}` 형태의 JSON으로 만들고, Bash로 `python3
> assets/monthly_campaign_rows.py`에 stdin으로 파이프해 호출한다(파일에 쓰지 않고 echo/heredoc으로
> 직접 파이프 — MCP 응답을 스크래치패드에 썼다가 다시 읽지 않는다는 원칙과 동일). 표준출력으로
