...
>   `threshold` 필터·정렬·`<tr>` 생성. 응답이 `[laighthouse-capture-hook] ... 저장됨: <경로>`
...
>   응답을 먼저 파일로 저장했다가 별도 호출로 다시 읽어 실행하는 2단계도 금지다. 빌더에
>   넘기는 것과 **같은** `metric_keys`/`currency`/`threshold`를 함께 넘긴다.
