...
> `group_by`가 `campaign`/`ad-set`/`ad`인 응답을 받으면(예: section-5) **반드시** Bash(grep/awk/jq
> 등, 파일로 남기지 않는 즉석 명령)로 전체 행에 대해 정확하게 필터링·합산·정렬한다 — 이건
> "해도 되는" 선택지가 아니라 **필수 절차**다. "이 정도만 훑어보고 나머지는 추정" 같은 부분
> 처리는 절대 금지한다.
>
...
> 재생성하지는 않는다** (위 6단계 참고) — 텍스트 재생성이 아닌 도구(파일 복사/치환)로
