# Monthly Section 1: 목표 달성 현황 (Target Achievement)

**report_type:** `monthly-detailed` (항상 포함). 이 카드는 **당월(대상 월) 목표 대비 진행
상황**을 보여준다 — `target_date`가 월말이면 그 달의 최종 진행률이, 월중이면 그 시점까지의
진행률이 자연스럽게 나온다. 모든 "매출"은 `get_ad_performance` 응답 행의 revenue 역할 키
...
- 응답 블록 중 `media_list`(디스커버리 결과)에 있는 매체와 대소문자 무관 일치하는 블록만
  ...
  리터럴로 열거하지 않는다. Organic(`media: null`)은 목표 개념이 없어 이 블록들과 무관하다.
...
- no-budget 메시지는 오류가 아니다 — "데이터 준비 중" 처리하지 않고 아래 N/A(null) 규칙대로 간다.
