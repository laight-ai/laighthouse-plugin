# Executive Creative Section 1: 최우수 소재 (ROAS 또는 클릭 / CTR, 최근 7일 기준)

**report_type:** `creative-summary` (항상 포함). **`chosen_media` 하나만 대상**이다(SKILL.md
매체 선택). 기준일 포함 **최근 7일을 통째로 합산**한 소재(개별 광고) 단위 **ROAS 1·2위**(매출 없음 모드:
**클릭 1·2위**)와 **CTR 1·2위**를 카드로 보여준다 (`creative-detailed` section-1과 동일 내용).
...
- 각 행: 소재(`campaign_name`+`ad_group_name`+`ad_name`)별 7일 합산 cost/impression/click/
  revenue 키(있을 때) 값 + 7일 합산 기준으로 이미 계산된 서버 비율 지표(% 값, 이름은
  봉투 `metrics` 참고) — 매출 조인이 필요 없다.
...
- 아래 선정 로직으로 ROAS(매출 없음: 클릭) 1·2위 + CTR 1·2위를 먼저 정한 뒤, 그 소재들(최대 4개, 중복은
  유니크하게) 각각에 대해 `name_query`로 호출한다 — 전체 소재를 조회하지 않는다.
- `source`는 디스커버리 행에서 `chosen_media`와 짝지어진 `source` 값이다(`sources_of`, SKILL.md
  매체 선택 1). 여럿이면 **전부** 소스마다 1회씩 — 소스를 골라내거나 제외하지 않는다. 소재
  메타데이터가 없는 소스는 에러 없이 `items: []`를 돌려주므로 빈 결과는 건너뛴다. 어느
  소스에서도 못 찾으면 썸네일은 `null`.
...
- 오른쪽 카드(두 모드 공통): CTR = click 키 ÷ impression 키 × 100 내림차순 1·2위
  ...
  써도 된다(×100 금지).
- 두 랭킹은 독립(같은 소재가 양쪽 1위여도 그대로 둔다).
...
```json
"s1": {
  "roas":  [ {"name": "<ad_name>", "value": 388.05, "thumbnail_url": "https://..."}, {...2위...} ],
  "ctr":   [ {"name": "<ad_name>", "value": 2.41, "thumbnail_url": null} ]
}
```
매출 없음 모드에서는 `roas` 대신 `click`:
```json
"s1": {
  "click": [ {"name": "<ad_name>", "value": 5321, "thumbnail_url": "https://..."}, {...2위...} ],
  "ctr":   [ ... ]
}
```

- `value`는 숫자 원본 그대로(ROAS/CTR은 % 소수 1자리, 클릭은 콤마 정수 — 포맷은 빌더가 한다).
  빌더 입력 최상위에 `metric_keys`를 넣어야 모드가 맞게 판정된다. `name`은 소재(광고) 이름만 —
  캠페인/광고그룹명을 덧붙이지 않는다.
