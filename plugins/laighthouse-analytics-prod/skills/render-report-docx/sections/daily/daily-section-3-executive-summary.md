# Daily Section 3: 성과요약 (Executive Summary) — DOCX 출력 스펙

> 데이터 스펙: 스킬 폴더 기준 `../../shared/sections/daily/daily-section-3-executive-summary.md` 를 **먼저** 읽고, 이 파일의 출력 스펙을 적용한다.
...
⚠️ **상위 제목(`daily_summary_heading`)은 만들지 않는다** — "{M}월 {D}일의 성과요약"과
"{M}월 {D}일 광고 성과 요약"을 둘 다 렌더링하면 같은 날짜를 가리키는 문구가 중복 표시된다.
소제목(`daily_summary_subheading`) 하나만 이 섹션의 제목으로 렌더링한다.
...
6. ⚠️ **문장 종결은 항상 개조식(~함/~음/~임/~됨 등)으로 쓴다** — "~습니다/했습니다/합니다"체를
   쓰지 않는다. 예: "ROAS는 155%로 저조했습니다." (❌) → "ROAS는 155%로
   저조함." (✅).
```
...
- (naver 분기) 이 섹션의 제목은 `daily_summary_subheading`("{M}월 {D}일 광고 성과 요약") 하나뿐이다
  — 별도의 상위 제목("{M}월 {D}일의 성과요약")을 추가로 렌더링하지 않는다 — 동일 날짜
  문구가 중복 표시되는 것을 막기 위함이다.
