...
### 7. 섹션 조합 단계의 "중간 파일 생성" 금지 강화 + 스켈레톤 게시를 필수 체크포인트로 승격

자매 스킬 `daily-detailed`(2026-08-11)에서 실제 프로덕션 실행 중 드러난 두 가지 실패 모드를
같은 구조를 공유하는 `mtd-summary`에도 선제적으로 적용했다 (실제 사고는 `daily-detailed`에서
관찰됐고, `mtd-summary`는 아직 같은 사고를 겪은 적은 없지만 동일한 지침 구조를 그대로 갖고
있어 같은 위험에 노출돼 있었다).

- **무엇을 바꿨나**:
  1. **중간 파일 생성 금지를 조합 단계까지 명시적으로 확장**: § 실행 방식 절대 지침의 "이
     스킬이 만드는 파일은 오직 최종 보고서 HTML 하나뿐이다" 문구가 원래 데이터 집계 단계에만
     적용되는 것으로 읽힐 수 있었다. `daily-detailed`와 동일한 문구로 "이 금지는 데이터 집계
     단계에만 적용되는 게 아니라 섹션 HTML을 조합하는 단계에도 동일하게 적용된다"를 추가하고,
     구체적 금지 예시(`section2.html`, `section4.html`, 임시 스테이징 HTML)를 들었다. 각
     섹션의 HTML은 그 섹션을 만드는 같은 턴에서 바로 최종 문서 조합에 이어 쓰고, Write는
     최종 저장 시점에 딱 한 번만 실행한다는 문장도 그대로 이식했다.
  2. **스켈레톤 선(先) 게시를 번호 붙은 필수 체크포인트로 승격**: 기존에는 "⏱ 긴 대기 없이
     스켈레톤을 먼저 보여준다"는 문구가 § 실행 방식 절대 지침 아래 별도 안내문으로만 존재하고,
     번호 붙은 § 실행 순서 목록에는 포함돼 있지 않았다. 이 안내문을 "자세한 절차는 아래 '실행
     순서' 3단계(필수 체크포인트)를 따른다"는 포인터로 축약하고, § 실행 순서의 **번호 붙은
     3단계**(target/achievement 호출 직후, 나머지 데이터 호출 전)에 필수 체크포인트로
     다시 넣었다. 뒤따르는 단계 번호(나머지 도구 호출 → Executive Summary → HTML 조합 →
     chart.js 인라인 → 렌더링/저장 → 완료 메시지)를 4~9로 한 칸씩 밀어 다시 매기고, 4단계
     (나머지 도구 호출)에 "섹션 데이터가 준비되는 대로 즉시 골격의 해당 placeholder를
     교체·재게시한다"는 문장을 추가했다. 다른 절에서 옛 단계 번호를 참조하던 곳(§ 병렬 호출
     지침의 "2단계/3단계"→"2단계/4단계", "2단계 먼저, 3단계 나중"→"2단계 먼저, 4단계 나중",
     § 완료 메시지 형식의 "7단계에서 저장한"→"8단계에서 저장한", § 보고서 골격의 "위 6단계
     참고"→"위 7단계 참고", 실행 순서 7/8단계 내부의 상호 참조)도 전부 새 번호로 맞춰 고쳤다.
- **왜**: `daily-detailed/CLAUDE.md`의 2026-08-11 항목에 기록된 실제 사고(예외 조항의 경계가
  "집계"인지 "조합"인지 모호하게 읽혀 모델이 조합 단계까지 파일 생성으로 확장한 사례, 스켈레톤
  게시가 "권장" 수준의 별도 안내문으로만 존재해 예산이 부족한 실행에서 가장 먼저 생략된 사례)가
  `mtd-summary`에도 그대로 재현될 수 있는 구조였다 — 두 스킬 모두 서브에이전트 없이 오케스트레이터가
  MCP를 직접 호출하고, 여러 섹션을 조합해 하나의 HTML로 저장하는 동일한 실행 모델을 쓴다.
  실측 사고가 아직 `mtd-summary` 자체에서 관찰된 것은 아니므로, 이건 "이미 검증된 교정을 구조가
  같은 형제 스킬에 선제적으로 이식"한 것이다.
- **검증 방법**: 다음 `mtd-summary` 실행에서 (a) target/achievement 응답 수신 직후 Artifact
  게시가 실제로 한 번 일어나는지, (b) 이후 섹션 완성 때마다 재게시가 일어나는지, (c) 실행 중
  `section*.html`/`section*.json` 등 최종 파일 외의 Write 호출이 없는지 확인한다.
- **영향받은 파일**: `SKILL.md`(§ 실행 방식 절대 지침, § 실행 순서 3~9단계, § 병렬 호출
  지침, § 완료 메시지 형식, § 보고서 골격의 단계 참조).

### 8. `group_by`가 `campaign`/`ad`/`ad-set`인 섹션 부재 — 아래 두 항목 적용 대상 아님

`daily-detailed`에서 최근 적용한 두 가지 최적화(조인+파생지표+변화율+색상+정렬+`<tr>` HTML
생성을 `assets/dxd_table_rows.py` 같은 검증된 asset 스크립트로 옮기는 것, 그리고 캠페인/광고
단위 응답에서 `media` 생략 통합 호출 여부를 카디널리티별로 재검토하는 것)를 `mtd-summary`에도
적용할 수 있는지 확인하기 위해 SKILL.md와 5개 섹션 파일 전체를 grep했다.

- **확인 결과**: `mtd-summary`가 쓰는 `group_by` 값은 전 구간에서 **`"media"`뿐**이다
  (`mtd-summary-section-1/3/4/5` 전부 동일한 `media` 생략 + `group_by:"media"` 공유 응답을
  쓰고, section-4/5는 별도 호출조차 하지 않는다 — 위 2026-08-09 항목 1번 참고). `group_by`가
  `campaign`/`ad`/`ad-set`인 호출이나, D-1/D-0 조인·CTR/CPA/ROAS 파생지표·변화율·색상·`<tr>`
  HTML 생성을 하는 섹션이 이 스킬에는 **애초에 없다** — `mtd-detailed`(상세 버전)의
  Campaign/Campaign별 성과 섹션이 `mtd-summary`(임원용 요약 버전)에서는 빠져 있는 구성 차이
  때문이다.
- **결론**: 두 항목 모두 적용 대상이 없어 건너뛴다. 새 asset 스크립트를 만들지 않았고, 어떤
  섹션 파일의 호출 수도 바꾸지 않았다.
- **검토했으나 변경 없음**: `mtd-summary-section-1-target-achievement.md`,
  `mtd-summary-section-2-executive-summary.md`,
  `mtd-summary-section-3-monthly-ad-performance.md`,
  `mtd-summary-section-4-revenue-trend.md`, `mtd-summary-section-5-channel-comparison.md`.
