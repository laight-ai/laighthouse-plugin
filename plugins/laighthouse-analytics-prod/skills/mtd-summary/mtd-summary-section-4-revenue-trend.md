...
(function(){
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      scales:{ y:{ min:0, ticks:{ callback: v => '₩' + Number(v).toLocaleString() } } }
    }
  });
})();
```
...
## 렌더링 규칙
- Y축은 항상 **0부터 시작**한다(`min: 0`). 눈금 간격·최대값은 고정하지 않고 6개월 실제 매출
  값(전체 매출/광고 매출)의 범위에 맞춰 Chart.js가 자동으로 계산하도록 둔다.
...
- **차트 아래 각주**: 세 개의 독립된 줄(`*`로 시작하는 문장 세 개)을 아래 **순서 그대로**
  ...
  2. `{TYPE_B_REVENUE_TREND_FOOTNOTE_DEFINITION}`: 항상 표시한다. 두 번째 줄이다. 다음 고정
     문구를 그대로 쓴다: `* '전체 매출'은 오거닉 매출과 광고 매출, 기타 매출(CRM, 제휴 마케팅 등)을 합친 전체 매출을 의미합니다.`
