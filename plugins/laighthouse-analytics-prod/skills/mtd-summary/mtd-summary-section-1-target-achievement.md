# Breezm Executive MTD Section 1: 목표 달성 현황 (Target Achievement)

**report_type:** `mtd-summary` — **브리즘(airbridge 기반) 전용** (항상 포함). 당월 1일~기준일
(MTD) 목표 대비 진행 상황 카드. 모든 "매출"은 Airbridge 귀속 매출 — `get_ad_performance` 응답 행의 지표
`매출_AB`다.
...
- section-3의 6개월 호출과 별개다 — 스켈레톤 선(先) 게시를 위해 의도적으로 합치지 않는다
  (SKILL.md § 병렬 호출 지침).
...
| `footnote` | 목표(예산 또는 매출) 없는 매체가 하나라도 있으면 `true` (고정 각주는 빌더가 넣는다 — 어떤 매체인지는 나열하지 않는 문구) |

- no-budget 메시지는 오류가 아니다 — `s1`을 빼지 말고 N/A(null) 규칙대로 값을 채운다.
- 일부 매체만 목표가 있으면 소진율이 얼핏 왜곡돼 보일 수 있다 — `footnote: true`로 갈음한다.
- 응답 rows의 실제 `media` 값이 상수(`Google`/`Meta`/`Naver`)와 다르면
  조용히 0을 만들지 말고, Executive Summary(`s2`)에 불일치 안내 불릿을 추가한다.
