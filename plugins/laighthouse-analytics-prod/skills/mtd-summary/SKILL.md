...
> **섹션 HTML을 조합하는 단계에도 동일하게 적용된다** — `section2.html`, `section4.html`,
> 최종 조합 전의 임시 스테이징 HTML처럼 섹션별 조각이나 중간 조합본을 별도 파일로 만들었다가
> 나중에 다시 읽어서 합치는 방식은 절대 금지한다 (자매 스킬 `daily-detailed`의 실제 프로덕션
> 실행에서 이 방식으로 섹션마다 파일을 만들다가 최종 저장/완료 메시지 전송 전에 툴호출 제한에
> 걸려 사용자에게 아무 결과도 보여주지 못한 사례가 있었다). 각 섹션의 HTML 문자열은 그 섹션을
> 만드는 같은 턴에서 바로 최종 문서 조합에 이어 쓴다 — 파일을 매개로 왕복하지 않는다.
> **최종 저장 시점에 딱 한 번**, 완성된 전체 HTML을 아래 "실행 순서"에 정해진 저장 경로에
> Write한다.
>
...
4. 나머지 `mcp__laighthouse__*` generic 도구를 호출해 각 섹션 수치 데이터를 가져온다 (각 섹션
   파일에 명시된 정확한 tool명 참고). 섹션 데이터가 준비되는 대로 즉시 위 3단계 골격의 해당
   placeholder를 교체·재게시한다 — 스켈레톤이 게시된 이후에도 섹션별로 점진적으로 채워지도록
   한다.
...
7. ⚠️ **`chart.umd.min.js`(약 208KB)는 `{CHART_JS_INLINE}` 자리에 항상 전체 내용을 그대로
   인라인한다.** 이 파일을 Read해서 그 내용을 `<script>{CHART_JS_INLINE}</script>` 자리에
   그대로 붙여넣는다 — 208KB(대략 5만 토큰)를 출력 토큰으로 새로 생성해야 하므로 느리지만
   (2026-08-09 확인), **정확성(차트가 실제로 렌더링되는 것)이 속도보다 항상 우선한다**.
   상대 경로 `<script src="chart.umd.min.js">`로 분리해 참조하는 방식은 쓰지 않는다 — 실제
   호스트 저장/프리뷰 환경에서 sibling 파일을 상대 경로로 못 불러와 차트가 전부 깨지는 사례가
   확인됐다(자세한 배경은 `CLAUDE.md` 참고). 항상 인라인하므로 "이미 파일이 존재하는지 확인
   후 건너뛴다"는 절차도 없다 — 매 리포트마다 `{CHART_JS_INLINE}`을 새로 채운다.
8. 아래 **보고서 골격**에 섹션들을 삽입해 렌더링한다. 완성된 HTML은 호스트가 지원하는 경로
   ...
   - **채팅 내부 표시** (Artifact 또는 `mcp__visualize__show_widget`이 있는 호스트에서만):
     Claude Code(Artifact)에서 실행 중이면 Artifact 도구로 게시(위 3단계 스켈레톤과 같은 파일을
...
- ⚠️ **이 스킬 전체가 데이터 호출 6개를 전부 두 배치(2단계/4단계)에서 동시에 발사한다**
...
> MIT license, 오프라인 자산)를 Read해서 그 내용을 `{CHART_JS_INLINE}` 자리에 항상 전체
> 인라인한다 (위 7단계 참고) — 상대 경로 `<script src="chart.umd.min.js">`로 분리 참조하지
> 않는다. 저장/프리뷰되는 호스트가 sibling 파일을 relative path로 못 불러오는 경우가 실제로
> 확인됐고, 그 경우 모든 차트가 깨진다(자세한 배경은 `CLAUDE.md` 참고) — 느리더라도 인라인이
> 항상 더 안전한 기본값이다.
