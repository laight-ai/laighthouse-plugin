...
명시한다 (section-5는 미분류 값을 Others로 합산).
...
2. **1차 배치 (한 메시지에 동시 발사)**: `get_target_progress_v2` ×3(google/meta/naver) +
   `get_ad_performance_monthly_table` ×1(media 생략, 당월 1개월, `day_offset`=target_date.day)
...
4. **2차 배치 (한 메시지에 동시 발사)**: `get_ad_performance_monthly_table` ×1(media 생략,
...
- section-3의 6개월 공유 응답을 section-4(airbridge 행)/section-5(마지막 2개월)가 재사용한다
