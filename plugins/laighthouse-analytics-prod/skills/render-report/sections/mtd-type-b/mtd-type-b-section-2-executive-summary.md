...
**report_type:** `mtd` — **브리즘(airbridge 기반, type-b) 전용** (항상 포함). 실무자가 오늘
...
1. 수치 데이터 수집 — 아래 항목 중 "신규 호출" 표시가 없는 것은 전부 다른 섹션 응답을 그대로
   재사용한다 (별도 재호출 없음):
   - mtd-type-b-section-1(목표 달성 현황)의 get_target_progress_v2 응답(google/meta/naver
   ...
   - mtd-type-b-section-4(일일 매출 현황)의 일별 전체 매출/광고 매출 응답과
     `list_promotions` 응답.
   - mtd-type-b-section-6(광고 매체별 현황)의 매체별 소진액/광고 매출/ROAS 응답.
...
3. **매체별 특이사항**: mtd-type-b-section-6 데이터에서 매체(Google/Meta/Naver Ads)별
   소진액·매출·ROAS 중 가장 눈에 띄는 사항 하나를 한 문장으로 쓴다.
4. **전체/광고 매출 특이사항**: mtd-type-b-section-4 데이터에서 전체 매출 또는 광고 매출 추이
