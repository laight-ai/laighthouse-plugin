...
<div class="card" style="margin-bottom:16px;">
  ...
  <div id="dailyChartPromoWrap" style="position:relative; margin-top:4px;"></div>
</div>
```
...
(function(){
  ...
  // 프로모션 브래킷 오버레이 렌더링 (2026-07-24 추가, 2026-07-24 "특이사항" 레이블 삭제)
  // 주의: dailyChartPromoWrap은 캔버스와 폭이 정확히 같은 형제 요소여야 한다 — 그래야
  ...
  const wrap = document.getElementById('dailyChartPromoWrap');
...
## 렌더링 규칙
- `daily_promotions_ranges`가 빈 배열이면 프로모션 오버레이 영역(`dailyChartPromoWrap`) 자체를
...
- 브래킷 라벨이 구간 너비보다 길면 `text-overflow:ellipsis`로 잘라낸다 (구간이 짧은데 프로모션명이
  길 경우 대비).
- ⚠️ **좌표계 주의**: `dailyChartPromoWrap`은 캔버스와 폭이 정확히 같은 형제 요소로 둔다.
