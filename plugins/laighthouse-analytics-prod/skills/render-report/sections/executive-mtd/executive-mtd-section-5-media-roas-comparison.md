  ...
  <div style="overflow-x:auto;">
    <table>
      <thead>
        <tr>
          <th>채널</th>
          <th style="text-align:right;">{prev_period_label} ROAS</th>
          <th style="text-align:right;">{curr_period_label} ROAS</th>
          <th style="text-align:right;">변동</th>
        </tr>
      </thead>
      <tbody>
        <!-- media_roas_comparison.rows 배열을 순회하며 아래 행 반복 -->
        <tr>
          <td>{channel_label}</td>
          <td style="text-align:right;">{prev_roas_fmt}%</td>
          <td style="text-align:right;">{curr_roas_fmt}%</td>
          <td style="text-align:right; color:{CHANGE_COLOR}; font-weight:600;">{CHANGE_LABEL}</td>
        </tr>
        <!-- 반복 끝 -->
      </tbody>
    </table>
...
- `change_pp > 0`이면 `CHANGE_COLOR = #16a34a`(초록), `CHANGE_LABEL = "+{change_pp}%p"`.
- `change_pp < 0`이면 `CHANGE_COLOR = #dc2626`(빨강), `CHANGE_LABEL = "{change_pp}%p"`
  (마이너스 기호 유지, 예: `-179.7%p`).
