...
**report_type:** `mtd-detailed` (항상 포함). 최근 6개월(당월 포함), 연-월 단위. 매출은 revenue
역할 키(`metric_keys.revenue`) 값.

> ℹ️ 차트 HTML/Script/축·tooltip 포맷(통화콤마, ROAS 축 min:0)은 전부 템플릿+빌더가 처리한다 —
> 모델은 아래 규칙으로 **6개월치 배열 3개**만 빌더 입력 JSON의 `s3`에 넣는다.

## MCP 도구 호출: `get_ad_performance` × 1 (`filters` 생략)
...
- **`filters` 생략** — 1회 호출로 월별·매체별(`media_list` 전 매체 + Organic `null` 행) 행을
  전부 받는다. 각 행에 `month`("YYYY-MM") 키와 cost/revenue/conversion 키 등 지표가 함께
  들어있다(별도 매출 응답 없음).
...
| 필드 | 값 |
|---|---|
| `ad_cost` | 월별: `media`가 `null`이 아닌 전 행의 cost 키 합 |
| `revenue` | 월별: `media`가 `null`이 아닌 전 행의 revenue 키 합 (광고 매출) |
| `roas` | 월별: 매출 ÷ 광고비 × 100 (광고비 0인 달은 `null`) — 매체별 행을 합산했으므로 행의 서버 ROAS를 더하지 말고 원자 지표 합으로 계산한다 |
