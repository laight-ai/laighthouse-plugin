...
> `get_target_progress_v2` ×3 응답과 monthly_table(`media` 생략) 응답을 그대로 재사용한다.

## 매체별 계산 규칙 (cost/revenue 독립 판단)

- **절대 규칙**: `광고_매출`은 목표 유무와 무관하게 **항상** 섹션 1이 받은 monthly_table
  (`media` 생략) 응답의 해당 채널 행(google ↔ `Google Ads`, meta ↔ `Meta Ads`, naver ↔
  `Naver Ads`) `airbridge_revenue`를 쓴다 — `get_target_progress_v2`의 `revenue` 행 `actual`은
  **절대 쓰지 않는다** (naver가 0을 반환하는데 Airbridge에는 매출이 존재하는 사례 실측됨).
- no-budget 메시지 매체 → `월_예산`/`예산_소진율` null, `소진액`은 같은 공유 응답의 해당 매체
  행 `cost`로 대체 (추가 호출 없음).
