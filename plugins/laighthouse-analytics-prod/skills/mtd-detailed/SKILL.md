...
브리즘(airbridge 기반) 하나뿐이다.** 이 스킬은 **MTD 보고서**(Google/Meta/Naver 광고 성과를 Airbridge에 기록된 매출과 엮어 보여주는 상세 MTD 보고서) 전용이며,
...
다른 보고서 종류를 선택하는 개념은 없다 — 호출되면 항상 MTD 보고서를 렌더링한다.
...
>
> ⚠️ **이 금지는 데이터 집계 단계에만 적용되는 것이 아니라, 아래 6단계의 "섹션 HTML 조합"
> 단계에도 동일하게 적용된다.** "집계에는 임시 스크립트를 안 쓴다"는 원칙을 지키면서도 정작
> 각 섹션 HTML을 이어붙이는 단계에서 `section2.html`, `section7_rows.json`, 최종본 조합 전
> 임시 스테이징 HTML 같은 **섹션별 조각 파일이나 중간 조합본을 만들었다가 나중에 다시 읽어
> 합치는 방식**은 금지 대상이다 — 이런 파일도 "이 스킬이 만드는 파일은 오직 최종 보고서 HTML
> 하나뿐이다"라는 위 원칙을 위반한다. 각 섹션의 HTML은 그 섹션을 만드는 같은 턴에서 바로 최종
> 문서 조합에 이어 쓰고, Write(또는 Artifact 게시용 파일 쓰기)는 스켈레톤 게시 시점과 최종
> 저장 시점에만 실행한다.
>
> ℹ️ **위 스크립트 금지 원칙은 재사용 가능한 파이프라인 파일을 만들지 말라는 것이다.**
> `get_ad_performance_daily_table`/`get_ad_performance_weekly_table`/
> `get_ad_performance_monthly_table`처럼 **날짜별(캠페인×기간) 행**을 반환하는 도구를
> `group_by`가 `campaign`/`ad-set`/`ad`로 호출한 응답을 받으면 **반드시** Bash(grep/awk/jq 등,
> 파일로 남기지 않는 즉석 명령)로 전체 행에 대해 정확하게 필터링·합산·정렬한다 — 이건 "해도
> 되는" 선택지가 아니라 이 스킬(예: 섹션 5)에서 행 수가 많아질 수 있는 호출마다 지켜야 하는
> 필수 절차다. (`get_ad_performance_range_table`은 예외다 — 캠페인/그룹당 이미 구간 전체가
> 합산된 행 1개만 반환하므로, 날짜별 재합산이 필요 없다. 섹션 7이 2026-08-09에 이 도구로
> 전환된 이유이자, 이 예외가 처음 명시된 계기다.) "이 정도만 훑어보고 나머지는 추정" 같은
> 부분 처리는 절대 금지한다. 만약 Bash
> 집계를 시도하다가 effort/시간이 부족하다고 느껴지면, **눈으로 훑어서 대략적인 결과로
> 넘어가지 말고**, 더 작은 단위로 나눠서(예: 날짜별로 쪼개서 합산한 뒤 합치기) Bash 집계를
> 끝까지 완료한다. 정확한 계산 없이 만들어낸 순위·합계·TOP-N은 이 보고서에 절대 포함시키지
> 않는다 — 차라리 그 섹션을 "데이터 준비 중"으로 표시하는 것이 근사치를 보여주는 것보다
> 낫다. 실제로 같은 호출 계열(`group_by:"ad"`)에서 "effort 예산이 부족해 눈으로 표를 훑어
> 백여 개 조합 중 일부만 확인했다"며 추정치로 넘어간 사례(2026-08-09, `creative-detailed`
> 실행)가 있었다 — 이런 부분 처리·추정 결과물은 이 스킬이 막으려는 바로 그 오류다. Bash
> 집계 결과로 나온 작은 요약표만 컨텍스트에 남긴다 — 원본 테이블 전체를 손으로 옮겨 적거나
> 머릿속으로 합산하지 않는다. 이건 재사용 가능한 스크립트 파일을 만드는 것과 다르다.
>
> ⏱ **스켈레톤 선(先) 게시는 권장 사항이 아니라 § 실행 순서의 번호 붙은 필수 체크포인트다**
> (아래 3단계 참고) — 전체 보고서를 다 만든 뒤 한 번에 저장/게시하려다 툴호출 예산이 바닥나
> 사용자에게 아무 결과도 못 보여주는 사고를 막기 위함이다.
...
1. 파라미터를 파싱한다. report_type은 `mtd-detailed`로 고정되어 있다.
2. target/achievement 수치를 호출한다 — `mcp__laighthouse__get_target_progress_v2`를
   ...
   **google/meta/naver 세 번**(`media`만 바꿔) 호출한다
   (`mtd-detailed-section-1-target-achievement.md` 참고). 세 매체 모두
   ...
   **`get_ad_performance_monthly_table`(`start_month`=`end_month`=당월,
   `day_offset`=target_date.day, `group_by:"media"`, **`media` 생략**)** 1회 호출로 가져온다 —
   `media`를 생략하면 이 호출 하나가 google/meta/naver의 소진액과 airbridge의 매출 실적을
   전부 함께 반환하므로, 목표 판정 결과를 기다렸다가 매체별로 추가 호출할 필요가 없다(날짜별
   행을 직접 합산하는 것보다도 훨씬 빠르다 — 섹션 파일의 대체 규칙 참고). 매출 실적(`기간
   매출`/`광고 매출`)도 목표 유무와 무관하게 **항상** 이 응답의 airbridge 행에서 가져온다 —
...
3. ⏱ **필수 체크포인트 — 스켈레톤 선(先) 게시.** 2단계(target/achievement 호출) 응답을 받는
   즉시, 나머지 섹션은 전부 "데이터 준비 중" placeholder(§ 데이터 부족 시 규칙과 동일한
   마크업)로 채운 전체 골격을 1차로 Artifact에 게시한다(Claude Code Artifact 도구 또는
   `mcp__visualize__show_widget`이 있는 호스트에서는 그걸 쓴다). 이후 4단계에서 각 섹션 데이터가
   준비되는 대로 같은 Artifact 파일을 갱신(재게시)해 placeholder를 실제 값으로 교체한다 —
   사용자가 빈 화면을 오래 기다리지 않도록 먼저 뼈대를 보여주고 채워나간다. 이 단계는 생략
   가능한 "권장"이 아니라 반드시 실행해야 하는 체크포인트다.
4. 나머지 `mcp__laighthouse__*` generic 도구를 호출해 각 섹션 수치 데이터를 가져온다 (각 섹션
   파일에 명시된 정확한 tool명 참고). 각 섹션 데이터가 준비되는 대로 위 3단계에서 게시한
   골격의 해당 placeholder를 즉시 교체·재게시한다 — 모든 섹션이 다 끝난 뒤 한 번에 게시하지
   않는다.
   - **generic 도구**(`get_ad_performance_daily_table`/`get_ad_performance_monthly_table`/
     `get_ad_performance_range_table`)만 쓴다 — naver 전용 도구는 브리즘에 적용되지 않으므로
     절대 쓰지 않는다.
   - ⚠️ **`group_by`가 `total`/`media`처럼 저(低)카디널리티인 호출은 매체별로 나눠 여러 번
     부르지 않는다.** 섹션 파일에 다르게 적혀 있지 않은 한 `media` 파라미터를 아예 생략한다
     — 생략하면 등록된 모든 매체(google/meta/naver/airbridge 등)가 한 응답에 함께 온다.
     이렇게 받은 응답 하나를 여러 섹션이 공유해서 재사용한다(각 섹션 파일의 "공유 응답
     재사용" 절 참고) — 섹션 1/3의 목표·월별 광고 성과 호출, 섹션 2의 신규 호출이 이 방식으로
     매체별 3~4회 호출을 각각 1회로 줄였다.
   - ⚠️ **단, `group_by`가 `campaign`/`ad-set`/`ad`처럼 행 단위가 세분화되는 호출에는 위
     `media` 생략 규칙을 적용하지 않는다.** 섹션 7(Campaign별 성과)은 `group_by:"campaign"`
     이므로 매체별 4회(google/meta/naver/airbridge) 호출을 그대로 유지한다 — `media`를
     생략하면 불필요한 매체 행까지 한 응답에 섞여 MTD처럼 긴 기간에서는 응답이 컨텍스트에
     담기 어려울 정도로 커질 수 있기 때문이다(자세한 이유는 섹션 7 파일 참고).
...
5. **section-2가 Executive Summary다.** `executive_summary` 텍스트는 `df_dify` MCP를 호출하지
   ...
   세부 규칙은 `mtd-detailed-section-2-executive-summary.md`에 적힌 대로 따른다.
6. 아래 표의 파일을 **순서대로 전부** import해 HTML을 조합한다.
7. ⚠️ **`chart.umd.min.js`(약 208KB)는 항상 리포트 HTML의 `<script>...</script>` 안에
   완전히 인라인한다** — 상대 경로 `<script src="chart.umd.min.js">`로 별도 파일을 참조하는
   방식은 쓰지 않는다. 실측(2026-08-09, `creative-detailed` 실행)으로 확인된 이유: 리포트를
   `/mnt/user-data/outputs/` 같은 샌드박스 출력 디렉터리에 저장하면, 플랫폼 프리뷰가 HTML과
   `chart.umd.min.js`를 **서로 별개의 다운로드 파일**로 취급해 상대 경로 `<script src>`가
   전혀 로드되지 않는다 — 그 결과 모든 차트가 빈 캔버스로 렌더링된다. "파일이 이미 존재하는지
   확인 후 존재하면 건너뛰고 상대 경로로 참조한다"는 방식은 이 실패를 감안해 **완전히
   폐기**한다. 대신, 이 파일을 Read해서 얻은 내용을 그대로 `{CHART_JS_INLINE}` 자리에 채워
   저장할 HTML의 `<script>...</script>` 태그 안에 넣는다(자기 응답 텍스트로 208KB를 다시
   타이핑/재생성하는 것이 아니라, 파일 복사·치환 등 텍스트 재생성이 아닌 도구로 옮긴다면 그
   방식을 우선 쓴다 — 그런 도구가 전혀 없는 호스트에서만 최후 수단으로 모델이 직접 옮긴다).
   Claude Code/claude.ai Artifact뿐 아니라 파일로 저장하는 경우도 동일하게 인라인한다 — 저장
   경로별로 다른 방식을 쓰지 않는다.
8. 아래 **보고서 골격**에 섹션들을 삽입해 렌더링한다. 완성된 HTML은 아래 **두 곳에 동시에** 낸다 —
   하나만 하고 끝내지 않는다:
   - **채팅 내부 표시**: Claude Code(Artifact)에서 실행 중이면 Artifact 도구로 게시(위 스켈레톤과
     같은 파일을 갱신) — 위 7단계대로 `chart.umd.min.js`를 인라인한 HTML을 쓴다.
     `mcp__visualize__show_widget`이 있는 호스트에서는 그걸 쓴다.
   - **파일로 저장**: 동일한 최종 HTML을 `~/Downloads/laighthouse-reports/브리즘_mtd-detailed_
     {기준_일자}.html` 경로에 그대로 저장한다 (디렉터리가 없으면 새로 만든다) — 이때도 7단계대로
     `chart.umd.min.js`를 인라인한 동일 HTML을 그대로 저장한다(별도 `.js` 파일을 만들지
     않는다). 파일명 예: `브리즘_mtd-detailed_2026-05-15.html`.
...
> 클라이언트 구현에 달려 있다"고 명시한다(`daily-summary` 스킬에서 실측 확인된 사실 —
> 배치 5개 호출이 완전 순차 5개 호출보다 빠르긴 했지만, 이는 "턴 사이 모델 사고 시간" 제거
> 효과이지 네트워크 동시 실행이 아니었다). 따라서 배치의 실제 효과는 **"턴 오버헤드 제거"**이지
> "네트워크 동시 실행"이 아니다 — 진짜 속도 개선은 배치보다 **호출 총 개수를 줄이는 것**(위
> `media` 생략 통합)에서 나온다. 그래도 배치는 공짜 이득이니 계속 유지한다 — 하나씩 순차
> 호출하면 호출마다 브랜드 권한 확인 왕복과 턴 지연이 누적되어 보고서 생성이 느려진다.
> "2단계 먼저, 4단계 나중"이라는 실행 순서상의 서술은 데이터 의존성이 아니라 설명 편의상 나눈
> 순서일 뿐이다 — 서로 의존하지 않는 호출은 실제로는 한 메시지에서 함께 발사한다. (3단계
> 스켈레톤 게시는 2단계 응답에 의존하므로 이 배치 논의와는 별개다.)
...
> HTML과(7단계에서 필요 시 1회만 만드는) `chart.umd.min.js` 사본뿐이다.
...
— by LaightAI
📁 C:\Users\minhyeok\Downloads\laighthouse-reports\브리즘_mtd-detailed_2026-05-15.html
```
...
| 순서 | 섹션 | Import 경로 |
|-----|------|------------|
| 1 | 목표 달성 현황 | `@import mtd-detailed-section-1-target-achievement.md` |
| 2 | Executive Summary | `@import mtd-detailed-section-2-executive-summary.md` |
| 3 | 월별 광고 성과 | `@import mtd-detailed-section-3-monthly-ad-performance.md` |
| 4 | 일일 매출 현황 | `@import mtd-detailed-section-4-daily-revenue.md` |
| 5 | Campaign 분석 | `@import mtd-detailed-section-5-campaign-analysis.md` |
| 6 | Channel별 예산 소진 현황 | `@import mtd-detailed-section-6-channel-budget.md` |
| 7 | Campaign별 성과 | `@import mtd-detailed-section-7-campaign-performance.md` |

`mtd-detailed`에는 **`매출 추이`나 `매체 성과 비교`(전월 vs 당월) 섹션이 없다** — 그 둘은
`mtd-summary` 전용 섹션이다. 대신 `mtd-detailed`는 일일 매출 현황(4번)과 Campaign 분석(5번)을 쓴다.
...
> 차트가 빈 캔버스로 남는다. **상대 경로 `<script src="chart.umd.min.js">`로 별도 파일을
> 참조하는 방식도 쓰지 않는다** — 실측(2026-08-09, `creative-detailed` 실행)으로, 리포트를
> 샌드박스 출력 디렉터리(`/mnt/user-data/outputs/` 등)에 저장하면 플랫폼 프리뷰가 HTML과
> `chart.umd.min.js`를 별개 다운로드 파일로 취급해 상대 경로가 전혀 로드되지 않고 모든 차트가
> 빈 캔버스로 남는 것이 확인됐다. 이 스킬 폴더의 `assets/chart.umd.min.js`(Chart.js v4 UMD
> 빌드, MIT license, 오프라인 자산)를 **항상** `{CHART_JS_INLINE}` 자리에 완전히 인라인한다 —
> 모델이 그 208KB를 직접 응답 텍스트로 재생성하지 않는다(위 7단계 참고): 텍스트 재생성이
> 아닌 도구(파일 복사/치환)로 `{CHART_JS_INLINE}` 자리를 채우는 것을 우선 쓰고, 그런 도구가
> 전혀 없을 때만 최후 수단으로 모델이 직접 붙여넣는다.
...
<head>
<meta charset="UTF-8">
<!-- chart.umd.min.js는 항상 아래처럼 완전히 인라인한다 (상대 경로 <script src>는 쓰지 않음 —
     샌드박스 출력 디렉터리에서 HTML과 별개 파일로 취급되어 로드되지 않는 것이 실측 확인됨):
<script>
{CHART_JS_INLINE}
</script>
-->
...
- 섹션을 임의로 생략하지 않는다 — 이 스킬은 7개 섹션 전부 항상 렌더링한다.
