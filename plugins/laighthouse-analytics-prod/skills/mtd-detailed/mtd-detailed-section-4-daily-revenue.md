...
2. **`labels` 배열은 항상 연속된 달력 날짜**(월초~target_date)이므로, 인덱스는 "찾기"가
   아니라 **날짜 차이를 직접 계산**해서 구한다 — 애매하게 "해당하는 인덱스를 찾는다"고만
   하면 정확한 날짜 매칭 로직 없이 잘못 계산될 수 있다. 정확한 계산법:
   - `raw_start_idx = (date_begin − labels[0]의 날짜).일수차`
   - `raw_end_idx = (date_end − labels[0]의 날짜).일수차`
   - `start_idx = max(0, raw_start_idx)`, `end_idx = min(labels.length-1, raw_end_idx)` —
     `raw_start_idx`가 음수면 0으로, `raw_end_idx`가 `labels.length-1`보다 크면
     `labels.length-1`로 클램프한다(차트 범위 밖으로 삐져나온 구간은 보이는 부분만 표시).
3. `date_begin`/`date_end`가 `labels`의 범위와 전혀 겹치지 않는(즉 `raw_end_idx < 0`이거나
   `raw_start_idx > labels.length-1`인) 프로모션은 제외한다.
...
- X축은 월초~기준일 **하루도 건너뛰지 않고 전부** 표시한다(`autoSkip: false`). 각 눈금은 위쪽에
  ...
  배열로 준다 — 위 "필요 데이터" 참고).
- Y축은 항상 **0부터 시작**한다(`min: 0`) — 값이 작을 때 축이 위쪽만 확대되어 등락폭이
  과장되어 보이는 것을 막는다. 눈금 간격·최대값은 고정하지 않고 그 기간 실제 매출 값(전체
  매출/광고 매출)의 범위에 맞춰 Chart.js가 자동으로 계산하도록 둔다(`stepSize`를 강제하지
  않는다). 값은 원 단위 숫자에 `₩` 접두어와 천 단위 콤마를 붙여 표시한다 (예: `₩15,000,000`)
  — 만원/억원 등 축약 단위로 바꾸지 않는다.
