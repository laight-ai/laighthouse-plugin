...
name: mtd-summary
description: >
  브랜드 비종속 Executive MTD 보고서 생성 스킬. "Executive MTD 보고서", "임원용 MTD 보고서", "executive mtd" 요청 시 사용.
...
MCP 데이터를 받아 **라이트하우스 스타일 Executive MTD 보고서**(HTML)로 렌더링한다. 다른
종류의 보고서는 각각 별도 스킬이다 (`mtd-detailed`/`daily-detailed`/`daily-summary`/
`monthly-detailed`/`monthly-summary`/`creative-detailed`/`creative-summary`). 이 스킬은 호출되면
항상 Executive MTD 보고서를 렌더링한다 — weekly는 지원하지 않는다(요청받으면 알맞은 스킬을
안내하거나 미지원임을 알린다).
...
| 파라미터 | 설명 | 예시 |
|--------|------|------|
| 보고서 제목 | 보고서 상단 타이틀 (`{브랜드명} Executive MTD 보고서`) | acme Executive MTD 보고서 |
...
```
{브랜드명} Executive MTD 보고서({기준_일자}) 생성 완료.
