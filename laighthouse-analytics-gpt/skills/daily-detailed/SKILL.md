...
> 경우는 존재하지 않는다). 응답을 못 받았을 때만 (2)로 간다. 이 환경(ChatGPT Skills)에는
> Claude Code의 캡처 훅이 없다 — 응답을 애초에 크게 만들지 않는 사전 예방 규칙은
...
> 사전 예방 규칙(대용량 응답을 애초에 만들지 않음, `gpt-large-response-guardrail.md`),
