# Executive Monthly Section 4: 매출 추이 (Revenue Trend)

**report_type:** `monthly-summary` (항상 포함). 최근 6개월(당월 포함) 라인 차트. 매출은
...
`day_offset`=target_date.day) 응답에서 월별 revenue 키 합(= 광고 매출)을 얻을 수 있다.
