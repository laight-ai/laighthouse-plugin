...
**권장 — 응답 원본 JSON 봉투를 그대로 넘긴다** (`json` 배열에 가공 없이 통째로; 캡처 훅
스텁으로 왔으면 대신 `json_files`에 저장 경로 배열을 넘긴다 — 판별 규칙은
`shared/references/gpt-large-response-guardrail.md` 참고). 빌더가 봉투 파싱·행 변환까지 전부
처리한다. 지표
