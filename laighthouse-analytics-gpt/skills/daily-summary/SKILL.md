...
> - 이 스킬의 호출은 전부 `group_by:["media"]` 저카디널리티라 응답이 작다 — 이 환경에는 캡처
>   훅이 없으므로 응답은 항상 원본 JSON 봉투 그대로 온다. 원본 문자열을 그대로 `json`에 넘긴다
>   (대용량 대응 원칙은 `shared/references/gpt-large-response-guardrail.md` 참고).
