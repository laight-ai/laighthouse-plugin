...
SKILL.md 실행 순서 3-b에서 호출한 `get_ad_performance`(`time_grain:"day"`,
`filters:{"media":["<chosen_media>"]}`, 소재 단위, 최근 7일) 응답을 그대로 쓴다 — 이 섹션에서
다시 호출하지 않는다. section-1의 total 응답은 날짜가 무너져 있어 이 섹션의 "날짜별 추이"에
쓸 수 없다.
