...
- section-1의 total 응답(소재당 7일 합산 한 행)과는 **grain이 다르다** — section-3/4는
  **날짜별 값**(일별 트렌드, 행마다 `date` 키)이 필요해 total로는 만들 수 없다. **section-4가
  ...
  발사한다. 매출(revenue 키)이 각 행에 지표로 들어있어 별도 매출 호출이 없다.
...
- 원본 JSON 봉투가 그대로 오므로 컨텍스트에서 바로 5개 키 행만 걸러낸다(별도 파일/스크립트
  불필요). 캡처 훅 스텁으로 왔으면 저장 파일을 python으로 읽어 걸러낸다. 어느 쪽이든 응답을
  통째로 재타이핑하거나 컨텍스트에 불필요하게 다시 옮겨 적지 않는다(판별 규칙은
  `shared/references/gpt-large-response-guardrail.md` 참고).
- 날짜별 `CTR` = click 키÷impression 키×100. 행 하나가 곧 소재×날짜 단위라 응답에 서버 계산
  CTR 지표가 있으면 같은 값이다(이미 % 값 — ×100 금지).
